...
/// Supabase string values shared with the web client. The cross-platform
/// fixture treats this as data so a spelling change must land on both clients
/// and in the database contract together.
enum SupabaseEnumContract {
...
struct MealComposerItem: Identifiable, Hashable, Sendable {
    ...
    var waterML: Double {
        max(0, waterML100 ?? 0) * max(0, equivalentAmount) / 100
    }
...
enum MealLogKind {
    static let acceptedValues = SupabaseEnumContract.values["logged_as"] ?? []
    private static let accepted = Set(acceptedValues)
...
enum ActivityInputStyle: String, Codable, CaseIterable, Sendable {
    case count
    case duration
    case distance
    case steps
