...
## 2026-08-27 — Urgent sync follow-up after 2.8

- Read the physical iPhone's persisted account-scoped outbox and quarantine files. The active account had no pending writes and exactly 20 quarantined failures: six `JWT issued at future` failures, three forbidden direct `logged_meals` updates, three workout logs whose parent session was missing, six legacy meal-kind constraint failures, and two duplicate-key failures. The red count was therefore a real quarantine count, not a paused or silently wedged retry queue.
- Classified PostgREST `PGRST303` and `JWT issued at future` as authentication recovery. At the start of a drain, only persisted authentication failures now move losslessly back to the durable outbox; permanent failures remain quarantined and visible.
- Replaced meal finish-time writes to the immutable `logged_meals` table with the existing atomic `log_structured_meal` replacement RPC, preserving the meal ID, metadata, and food-entry snapshots without counting a clock correction as another Food Memory use.
- Workout completion now commits the session parent, workout-log children, and linked activity as one atomic parent-first outbox bundle before draining. Stored array order is authoritative across reloads, and every replay step is account-generation and owner gated, so neither timestamp ties nor an account switch can race or quarantine another account's work.
- Replaced the unexplained red footer label with a shared 44-point accessible button and localized Sync Issues sheet in both Simple and Advanced modes. Each stored failure shows the affected domain, a plain-language explanation, app-locale timestamp, and an expandable exact technical reason. Failure details clear at account boundaries and are owner-validated after storage awaits. The new copy is authored in all nine offered languages.
- Verification: five focused sync unit regressions passed; the full native suite passed 524/524; the sync-details UI regression passed; localization contracts passed 6/6; structural localization coverage passed 4,561/4,561 in all nine languages; all eight edited string tables passed `plutil`; the repository suite passed 592/592; the production web build succeeded; and `git diff --check` passed. Apple build number advanced from 367 to 368.

## 2026-08-27 — Native-authored voice corpus follow-up after 2.7

- Defined a bounded first voice corpus of 89 context-and-meaning records covering Session Briefing, Activity Guide explanations, progress-camera guidance, daily nudge explanations, Avatar explanatory tooltips, and every exercise-specific coaching cue plus fallback. Copy was authored and read back per language rather than translated sentence-by-sentence for English, German, Swiss Standard German, Italian, Spain-oriented Spanish, pt-PT, Japanese, Romanian, and Thai.
- Materialized 712 reviewed runtime rows across the eight translated tables from locale-owned source modules. The corpus keeps screen context and intended meaning beside every key, validates exact key order and format placeholders, and owns the final override position so later mechanical generation cannot replace the reviewed Romanian or Thai prose.
- Replaced the six-language English fallback in `ExerciseGuidance` with explicit lookup in the user's selected language bundle. Romanian and Thai duplicate Swift fields were removed, leaving the authored string tables as the single runtime source for all non-English coaching cues.
- Added regression contracts for corpus provenance, exact runtime-table equality, generator precedence, and reviewed calibration phrases in every translated language. The calibration locks the support-muscle label, the mobility aphorism, and the breathing cue without presenting structural checks as evidence of prose quality. Two native tests prove both a named exercise and fallback guidance resolve independently in every offered language.
- Stabilized the existing Food Memory behavioral regression against SwiftUI's intentional propagation of each `meal-item-card-*` accessibility identifier. The test now combines that propagated identifier with the exposed accessibility label, and derives the exact Swiss rolled oats card identity before selection; meal behavior and product copy are unchanged.
- Verification: authored-copy contracts passed 4/4; focused localization policy/runtime checks passed 13/13; the full native unit suite passed 526/526; the Food Memory behavioral UI regression passed 1/1; the repository/web suite passed 596/596; the production web build succeeded; structural localization coverage remained 4,561/4,561 in all nine languages; all eight edited string tables passed `plutil`; `git diff --check` passed; and independent review found no Critical or Important issues. Apple build number advanced from 368 to 369.

## 2026-08-28 — Watch complication current-state refresh follow-up

- Traced the face/app percentage split to competing stores: the complication could publish a partial HealthKit aggregate over the newer canonical phone snapshot, while delayed samples and deletions made last-write-wins timestamps insufficient.
- Made the phone snapshot authoritative when present, with HealthKit used as a scoped overlay/fallback. A persisted anchored query now supplies explicit additions and deletion IDs, and owner/day/generation guards prevent stale asynchronous work from crossing account or midnight boundaries.
- Phone changes now send durable application context plus an immediate current-complication transfer. Fractional revisions, causal disconnect tombstones, sample-ID deduplication, and immediate WidgetKit reloads keep the face and opened app on the same visible state.
- Added a revision-scoped widget sidecar so the complication never overwrites canonical shared state. Local adds/deletes remain visible while syncing, and a durable owner/day-scoped delete queue retries until the phone acknowledges the deletion.
- Verification: 32/32 focused hydration/complication tests, 547/547 native tests, and 596/596 repository tests passed; the production web build and aggregate Watch simulator build succeeded. Signed Release 370 passed deep signature validation and installed on the physical iPhone; the final Watch app also installed, launched, and rendered on the watchOS simulator. Physical launch was unavailable because the sleeping iPhone was locked and the paired Watch developer tunnel timed out.

## 2026-08-28 — Hydration timeline chronological colour order

- Confirmed hydration composition is intentionally newest-first so the silhouette paints the latest addition at its top. The iPhone progress line reused those vertical stops horizontally, which put the newest preset colour on the left instead of the right.
- Added an explicit chronological timeline layout that reverses only horizontal composition bands while preserving exact millilitre proportions and softened boundaries. Both the iPhone quick-add line and Watch horizontal gleam now run oldest-left to latest-right; both vertical silhouettes retain newest-at-top order.
- Added native proportional-boundary coverage and updated the Watch source contract to distinguish horizontal and vertical orientation instead of requiring the buggy shared order. Independent review found no remaining Critical or Important issue.
- Verification: 33/33 focused hydration layout tests, 548/548 native unit tests, and 596/596 repository tests passed; the production web build and Watch simulator build succeeded. The final Watch app installed and launched on the watchOS simulator. Signed Release 371 passed deep signature validation and installed on the physical iPhone; physical launch was unavailable because the sleeping handset was locked, and the paired Watch developer tunnel timed out.

## 2026-08-28 — Active-account sync quarantine reconciliation

- Re-read the physical iPhone's persisted account-scoped files after installing the candidate build. The active account had an empty outbox and exactly 14 quarantined permanent failures, so the queue was neither paused nor stuck: six legacy `log_structured_meal` calls rejected `logged_as = actual`, one meal RPC collided on reused entry IDs, one health metric collided on its key, three workout logs referenced a missing session, and three obsolete direct meal writes were denied.
- Cross-checked all 14 against the durable local dashboard and current server dashboard before clearing anything. The seven meal RPCs' nine entry IDs now belong to the successful current breakfast; the health value has a newer same-day measurement; the missing workout and parent are absent from both durable snapshots; and the three direct meal edits are superseded by the current atomic replacement. A fresh post-install quarantine copy matched the original audit byte-for-byte before mutation.
- Added conservative account-scoped reconciliation after a successful server dashboard load. Exact meal and health rows can retire safely; invalid legacy meal kinds normalize to `custom`; only the evidenced session foreign-key failure can retry; ambiguous duplicates, different values, missing workout parents, malformed payloads, and unrelated foreign keys remain visible. Repaired operations keep their original IDs and timestamps, re-enter the durable outbox ahead of newer work, and the outbox is written before quarantine removal.
- Refresh recovery now tracks the exact repaired operation IDs. It reloads authoritative server state only after those IDs are confirmed absent from both outbox and quarantine, preserves the cached optimistic dashboard for paused, failed, or unknown replay state, and permits at most one follow-up load. A re-quarantined operation therefore cannot recurse indefinitely or masquerade as a successful drain.
- Cleared only the audited active-account quarantine on the physical iPhone. Read-back verification reports active quarantine **0** and active outbox **0**. The inactive account remains untouched at **47** failures and retains its original SHA-256. The complete 14-record pre-clear copy remains recoverable in the local device-audit directory.
- Verification: 15/15 focused sync tests, 19/19 adjacent meal/queue tests, 558/558 full native unit tests, and 596/596 repository/web tests passed; the production web build succeeded; `git diff --check` passed; and independent review found no remaining Critical or Important issue. Signed Release 372 passed deep signature validation and installed on the physical iPhone. The locked handset denied launch; the final app installed and launched successfully on the iOS Simulator.

## 2026-08-28 — Roadmap 1.10: session briefings that teach something true

- Added a movement-knowledge engine that classifies the session's canonical movements into strength/bodyweight, mobility, yoga, isometric, carry, cardio, interval, and recovery families. It also derives before/after-session position, completed-session experience bands, and persisted pain, joint, and hypermobility context.
- Connected the knowledge to the real muscle-map briefing for the exact selected workout date. Briefings now teach up to three movement-specific lessons, add session-context guidance, and surface ordered cautions without replacing the existing personalized muscle and recovery briefing.
- Authored the complete new coaching prose independently for German, Swiss German orthography, Spanish, Italian, European Portuguese, Romanian, Thai, and Japanese. Added a calibration contract that protects native phrasing and myth negation, alongside exact-key coverage.
- Added `docs/training/MOVEMENT_SCIENCE_LEDGER.md` with claim confidence, limitations, and primary evidence for every new training, mobility, recovery, and safety statement.
...
## 2026-08-28 — UI-fixture production-sync isolation and RLS refresh repair

- Re-read the physical iPhone's account-scoped persistence instead of inferring from the UI. The active account remains clean at **0 pending** and **0 quarantined** operations. The **47** retained failures belong to the signed-out account and remain untouched.
- Traced the fresh APEX/372 production failures to the iOS 26.5 Simulator: their `Darwin/25.5` user agent does not match the physical iPhone's Darwin 27.0 runtime. Ordinary `-apex-ui-test` fixtures were local at bootstrap but several persistence paths could still enqueue or send remote writes afterward.
- Added a debug-only remote-mutation firewall at every Supabase write boundary and made all local UI-fixture persistence paths remain local, including direct meal, preset, progress-photo, and completed-workout transaction paths. The firewall's first red workout regression exposed one remaining queued workout replay; completing a fixture workout now saves its local snapshot and receipt without creating or draining an outbox.
- Preserved real authentication recovery for supabase-swift errors that omit HTTP status. SQLSTATE `42501` with an anonymous row-level-security message now requests the bounded token refresh path, while a genuine `permission denied` `42501` remains a permanent authorization failure.
...
## 2026-08-28 — Constantine V8.3 and finished-workout retention hotfix

- Read the connected iPhone's account-scoped dashboard and outbox files rather than inferring from the interface. Constantine remained the authenticated owner of `Constantin Training V8.3` with protocol version `83`; June's programme remained independently owner-scoped.
- Found the exact Friday defect: authored V8.3 day `52429D97-DEA9-49AF-B4BC-F678AD447417` (`Legs B · lunge day + Focus T25`) was present and owned by Constantine but remained in `training_induction_archived_day_ids`. That identifier had once been generated deterministically and was later deliberately reused by the guarded V8.3 maintenance repair, so identifier-shape inference incorrectly treated the authored Friday as disposable generated history.
- Added durable original-programme protection on native and web. Existing authored day IDs are captured before a questionnaire plan is installed; protected IDs outrank stale archive markers; restoration archives only generated overlays; Settings keeps `Restore my original programme` available; and the repair persists back through the ordinary owner-scoped settings sync. Explicit Constantine and June manifests cover the historical collision while generic inference protects other accounts without reviving generated or foreign rows.
- Compared current source with the last accepted pre-20:00 implementation (`5caa41f7f0a594b0996ce7232e7dc9b3c229cb4d`, followed by clean handoff `2eb5e496cf5fab8f0053842ca619d43f5d71d9d2`). Finished-workout cards, swipe-to-delete, expanded deletion, receipt editing, and Settings recovery had not been removed. They appeared missing because Simple, Main and Transition filtered receipts to the selected calendar date. Those surfaces now show the five most recent owned completed workouts across dates and include the recorded date; Nutrition's deliberately selected-date activity history remains date-scoped.
- Added a load-bearing roadmap retention rule: no task may silently remove, replace, hide or reset a shipped programme, owner data, workflow, modality or recovery control. Every release must retain installed-plan weekday resolution/recovery, finished-workout visibility, swipe deletion, receipt editing and account isolation unless the owner explicitly directs removal.
...
## 2026-08-29 — Premium information hierarchy Task 2: inline Fitness Plan disclosure

- Reordered Advanced consistently on native and web to Profile/Avatar, Nutrition, Fitness Plan, Custom Workouts when available, then Orbit. Transition and Main phase journeys now live inside one inline Fitness Plan disclosure instead of competing with the primary destinations.
- Implemented the planned first-use sequence: Transition guidance appears first, Main guidance follows, and `fitness_plan_intro_seen` persists only after both cards have appeared. Subsequent expansions replace guidance with separate information controls. Dismissal, outside-tap dismissal, reduced-motion timing, timer cleanup, and account-settings persistence are covered without changing either phase journey.
- Kept compact visible labels separate from complete accessibility copy. The disclosure and both phase cards expose stable identifiers, expansion state, full VoiceOver labels, 44-point information controls, and deterministic ordering.
- Added authored full and constrained copy in every offered native language: English, German, Swiss German orthography, Spanish, Italian, Japanese, European Portuguese, Romanian, and Thai. Web English, Romanian, and Thai copy is authored in the shared translation table. Exact-value localization tests now reject missing keys, fallback copy, or merely increased counts.
- Updated all affected UI journeys to deliberately expand Fitness Plan and strengthened the Advanced smoke journey to prove the collapsed default, staged guidance, persisted later state, and information controls.
- Browser verification proved the complete five-item order with a custom programme, delayed first-use staging, persistence only after both cards, immediate reduced-motion reveal, later information controls, outside dismissal, accessibility state, and zero console errors.
- Verification: full web/repository suite 618/618; production web build succeeded across 1,183 modules; all 17 full/compact string tables passed `plutil`; complete native run 606/606, including all unit/localization tests and all 20 UI tests; focused Advanced journey 1/1; `git diff --check` passed. All simulator verification ran by explicit UDID on the dedicated `APEX Lane · iPhone 17 Pro`; the reserved BA-Studio and Finalova lanes were not used.

## 2026-08-29 — External HealthKit history and hydration complication hotfix

- Imported authorized historical and newly observed `HKWorkout` records through anchored and observer queries, with bounded pagination, stable UUID identity, account ownership, source/date/duration/energy/distance receipts, hidden-from-APEX semantics, and complete HealthKit activity-name coverage. APEX-authored HealthKit mirrors remain deduplicated without weakening native receipt correction or deletion.
- Kept HealthKit active energy authoritative so imported workout calories are descriptive and never added into the daily energy total a second time. Fitness, Orbit, calendar, and web consumers now share strict owner, visibility, source-bundle, metadata, and start-time matching contracts.
- Made the owner/day/revision-guarded App Group snapshot canonical for the Watch app and complication. Phone transfers, local Watch additions/removals, goal changes, app activation, and midnight rollover invalidate only changed timeline state; delayed older state cannot win, and unchanged values cannot create a reload loop.
- Added production migration `032_external_healthkit_workouts.sql` idempotently to the linked Supabase project. Remote verification found all ten new columns, three constraints, and two indexes.
- Fixed the final physical-build packaging regression that removed both the Watch launcher icon and the in-app body: the APEXWatch target now compiles the existing watchOS `AppIcon` and exact canonical `HydrationMaleSilhouette` catalog. The focused contract first failed on the missing resources phase, then passed after the target fix.
- Verification: full repository/web suite 638/638; production web build succeeded; focused native hotfix suite 128/128; full native unit suite 628/628; complete iOS UI suite 20/20; all 19 string tables passed `plutil`; Watch asset contracts 9/9; `git diff --check` passed. Signed Release 380 passed deep/strict validation and its Watch catalog contains named `AppIcon` and `HydrationMaleSilhouette` renditions.
- Physical-device proof: exact Release 380 installed on `iConstantine Main` and `Constantin’s Apple Watch`. The Watch app visibly rendered the restored silhouette at 1.70 L / 45%, and the watch-face complication immediately rendered the same 45% instead of the stale 33%. Production import then contained natural Apple Watch HIIT, run, walk, cycling, strength, core, and other receipts with real UUIDs and available metrics; a grouped production query found zero duplicated HealthKit workout IDs.

## 2026-08-29 — Training phase hierarchy and viewport-owned stat fills

- Cleared the top of installed Transition and Main phases by moving the plan-rebuild panel to the true bottom of each page and renaming its action to `Build a new plan`. The working hierarchy is now Today's Signal, today's prescribed workout, Full session / Minimum effective, calendar, then finished history and the remaining phase tools.
- Replaced the phase headlines with `Fitness initiation` and `It's time to transform, no excuses`, with authored German, Swiss German, Spanish, Italian, Japanese, European Portuguese, Romanian, and Thai copy. Web English, Romanian, and Thai parity includes the same phase hierarchy, holographic signal, mode selection, and rebuild placement.
...
## 2026-08-29 — Linked wearable workout evidence and player recovery

- Added a persistent, account-owned link from an imported HealthKit workout to its APEX workout session. Automatic matching accepts one unambiguous external activity that starts from five minutes before the APEX session through the APEX end and overlaps it; competing candidates remain unlinked for deliberate user choice.
- Added the guided-player `Already finished?` recovery flow. Opening it pauses the player, cancelling resumes it, and the person can either attach a same-day external workout from a newest-first list or finish without a wearable. External completion records only the facts that actually exist and no longer invents planned sets, duration, weight, or activity calories.
- Nested linked wearable evidence inside the APEX Finished Workouts receipt and suppressed its duplicate standalone card. Linked external energy remains evidence rather than a second fitness/activity signal, refreshes preserve the relationship, and deleting the APEX receipt only unlinks the original HealthKit workout.
- Kept automatic and manual linking owner/day scoped, excluded APEX-authored HealthKit mirrors, and preserved orphan external receipts, existing APEX editing/deletion, missing-metric behavior, and `Hide from APEX` semantics.
- Authored all new full and compact copy in every offered native language and added the corresponding Romanian and Thai web copy.
- Repaired a hierarchy accessibility regression exposed during verification: the signal-card identifier no longer overrides the nested session-briefing control, so both the premium order and movement-knowledge briefing remain operable.
- Verification: web/repository suite 649/649; production web build succeeded across 1,185 modules; native unit suite 638/638; focused automatic/manual linking, ownership/day, localization, premium-order, player recovery, and session-briefing journeys passed; all 17 localization tables passed `plutil -lint`; `git diff --check` passed.
- Physical verification: the exact signed Release build succeeded, validated its embedded Watch binary, installed and launched on `iConstantine Main` and `Constantin’s Apple Watch`, and both processes were confirmed running. A post-install physical capture showed the iPhone app live and the Watch hydration screen at 3.02 L / 81% with the restored APEX silhouette.
- Simulator isolation: native automated verification used only `APEX Lane · iPhone 17 Pro`. BA-Studio, Finalova, and the generic Watch simulator were not touched.

## 2026-08-30 — Workout insights and exportable evidence cards

- Added account-owned workout insights to Simple Mode and both training phases with day, week, year, and custom ranges. The summaries count completed APEX receipts and standalone imported activities once, nest linked wearable evidence into its APEX workout, exclude hidden/mirrored/foreign/out-of-range records, and leave genuinely missing energy, distance, or volume absent instead of inventing zeroes.
- Added at-a-glance workouts, active days, recorded time, active energy, reps, sets/efforts, recorded volume, and distance. Multi-year native summaries now page through all owned workout sessions and logs instead of silently stopping at the former 180/2,000-row dashboard limits.
- Added a rounded, high-resolution PNG card rendered locally on device, a native share action, and tenure-aware one-, five-, and ten-year anniversary treatment that appears only when the selected evidence range actually reaches that milestone.
- Authored the complete full and compact copy in every offered language, including localized duration units, and preserved wrapping for athlete names, metric values, and units.
- Verification: full web/repository suite 655/655; production web build succeeded across 1,188 modules; native unit suite 642/642; the dedicated native year-range/export/share UI journey passed; all 17 localization tables passed `plutil -lint`; `git diff --check` passed.
- Physical verification: the exact signed Release app passed strict code-sign validation, installed over the existing app without clearing account data, launched successfully on connected `iConstantine Main`, and a post-install capture confirmed the current authenticated APEX surface was live.
- Simulator isolation: all simulator work used only `APEX Lane · iPhone 17 Pro` (`6907359A-18D1-46B0-87F1-13CED5CE1C46`). BA-Studio, Finalova, and the generic Watch simulator were not touched.

## 2026-08-30 — Date-owned HealthKit history and resilient Food Memory hotfix

- Fixed the apparent 300-plus-workout flood at its source. Simple Mode had requested unbounded finished-workout history beneath every selected calendar day, so the same 311 legitimate historical HealthKit imports appeared on past, current, and future dates. Native and web now pass the selected date into the shared history resolver; phase history remains deliberately all-time.
- Added `Hide from APEX` to imported workout cards on native and web, including swipe reveal and confirmation, without deleting or editing the original Apple Health workout. Production inspection found exactly one visible HealthKit workout dated 29 August for Constantine; that row is now reversibly hidden in APEX, with `0` visible and `1` hidden for that date. No other historical workout was altered.
- Made Food Memory useful during provider outages. Native search now resolves account-owned foods, localized names, personal names, aliases, punctuation, diacritics, and token order offline, preserves hidden-food preferences, merges local results first, and no longer discards them when remote lookup fails. All native food-selection surfaces display the authored localized food name.
- Added authenticated database function `search_food_catalog` and deployed Food Lookup version 2. The function searches public catalogue rows plus the current account's private foods and preferences before consulting Open Food Facts; a provider 503 now returns a valid empty provider result instead of failing the whole request. Migration 033 also adds six provenance-preserving USDA FoodData Central burger references with authored names in every offered language. This small outage baseline does not replace or begin the separately deferred full Food Knowledge Corpus ingestion.
- Repaired the UI verification harness exposed by the Nutrition changes: long pages now scroll in settled bounded steps instead of queueing full-screen flicks, iOS 26 reachability replaces invalid ScrollView frame intersection, and the completed-workout fixture uses a deterministic same-day 18:00–18:30 interval so midnight cannot remove the workout and recovery context from the dayline.
- Verification: full repository/web suite 658/658; production web build succeeded; focused web history 19/19 and food schema 7/7; native history and Food Memory regressions passed; native unit suite 645/645; the complete native aggregate passed 668/668 with zero failures or skips; `git diff --check` passed. All simulator work used only `APEX Lane · iPhone 17 Pro` (`6907359A-18D1-46B0-87F1-13CED5CE1C46`); BA-Studio and Finalova were not used.
- Device release: signed Release 381 and its embedded Watch app passed deep/strict signature validation. Release 381 installed and launched successfully on connected `iConstantine Main` without clearing account data. The paired Watch was booted but temporarily unreachable over its local-network developer tunnel during the direct companion retry; the iPhone installation retains the correctly signed embedded Watch app for companion installation.

## 2026-08-30 — Food Knowledge Corpus roadmap project

- Added the separately gated Food Knowledge Corpus project to `docs/ROADMAP.md` from the external 1.8 GB source bundle without copying or committing any raw dataset.
- The roadmap now makes licensing, attribution, versioning, checksums, deterministic parsers, unit/basis preservation, trace/missing-value semantics, preparation state, provenance, evidence-backed deduplication, source-specific priority, multilingual aliases/barcodes, typo-tolerant retrieval, server-side storage, bounded account-scoped caching, source-by-source validation and preservation of every user-owned food/meal/history record load-bearing requirements.
- Recorded the owner’s later authorisation to begin execution after the urgent 30 August hotfix while retaining publisher-specific gates for FAO redistribution, Open Food Facts ODbL separation, FOODfiles terms, NEVO’s licence form and paid BLS data.
- Verification: the documentation diff contains no source data, `git diff --check` passes, and the repository worktree contains no file from `/Users/jaxoncorrey/Downloads/Food Facts for CB App`.

## 2026-08-30 — Food Knowledge Corpus ingestion foundation

- Added an additive, row-level-secured server schema for source manifests, ingestion batches, canonical food records, multilingual names and aliases, evidence-preserving nutrient observations, and bounded search projections. The authenticated search routine returns only validated, active batches and keeps source, market, preparation, basis, and macro provenance attached.
- Added a checksum- and licence-gated registry for every supplied source family. USDA, Swiss, UK, French, Danish, Japanese, Australian, Swedish, Canadian, Norwegian, Finnish, and New Zealand sources are approved for isolated source-by-source staging; Open Food Facts remains ODbL-isolated and the FAO bundles remain quarantined pending redistribution review.
- Added deterministic adapters for every approved source and external-only staging. Trace, below-detection, not-measured, and missing evidence remains null with its source text; grams and millilitres remain distinct; no missing nutrient or calorie is invented.
- Verified all 23 primary and companion artifacts byte-for-byte. Real-source smoke staging passed for all 15 approved datasets, including separate Swedish and Canadian nutrient files; the filtered USDA branded run produced 66 fast-food records and 983 evidence observations without committing raw or staged data.
- Deployed migration 034 to the linked Supabase project and confirmed all six corpus tables have row-level security enabled and `food_corpus_search_catalog` is live.
- Verification: repository/web suite 664/664; production web build succeeded across 1,188 modules; Python importer/adapters compile; `git diff --check` passes.

## 2026-08-30 — Food Knowledge Corpus production rollout

- Completed deterministic adapters, validation, staging, and reversible batch activation for all 15 currently approved national/US source bundles plus the official Wingstop nutrition guide. Publisher identifiers, multilingual names, preparation state, basis, units, original evidence text, trace/below-detection/not-measured/missing states, and source checksums remain preserved; duplicate publisher identifiers are disambiguated without merging different foods.
- Activated 16 source/version batches in the linked project with 42,818 canonical records and 42,818 bounded search projections. Live probes return McDonald's Big Mac/Side Salad/Hamburger, Burger King Hamburger/Cheeseburger/fries, and official Wingstop Old Bay wings/thighs/tenders with the available published macros.
- Preserved all 3,492,591 detailed nutrient observations in 16 deterministic gzip archives (51,784,657 bytes) in the private `food-corpus-evidence` bucket. Each active source records its archive object path, exact SHA-256, format, and observation count. The bounded database is 92 MB, contains no row-wise evidence duplicates, and the temporary HTTP transfer extension was removed after import.
- Added an additive serving-aware search RPC. Canonical Wingstop rows retain their original `per_serving` basis and published gram weight; the existing Food boundary derives a compatible per-100-g view only when that exact gram weight exists, keeps `1 serving = N g`, and rejects unweighted or unsupported bases instead of mixing units or inventing values.
- Food Lookup version 5 is active and merges account/local catalogue results with the canonical corpus before the separately attributed Open Food Facts provider. Open Food Facts remains ODbL-isolated; FAO and other publisher-gated sources remain quarantined pending their exact redistribution clearance.
- No raw source, generated staging payload, archive, or temporary-transfer file entered the repository or either client bundle. Existing private foods, aliases, preferences, meals, portions, scans, recents, and immutable history were not rewritten.
- Verification: corpus importer/edge/loader contracts 19/19; full repository/web suite 677/677; production build succeeded across 1,188 modules; Python importer/adapters compiled; `git diff --check` passed. Remote verification found 16 active/private archives, 42,818 records/search rows, 3,492,591 archived observations, 0 row-wise nutrient rows, 0 temporary HTTP extensions, and Food Lookup version 5 `ACTIVE` with JWT verification enabled.

## 2026-08-30 — Food Memory amount-sheet typography hotfix

- Fixed the Food Memory amount sheet shown in the owner capture. Five fixed-width macro tiles had forced long values such as estimated water into roughly 55-point lanes, while the portion receipt forced every metric into one non-wrapping row.
- Replaced the phone layout with readable adaptive macro columns that naturally expand on wider devices and become single-column at accessibility Dynamic Type sizes. The portion receipt now selects a single row, compact grid, or vertical list according to the actual available width.
...
## 2026-08-30 — Bespoke programme, Swiss fast-food, and Workout Insights presentation hotfix

- Moved Workout Insights beneath Finished Workouts in Simple Mode and both training phases, preserving the existing account-owned history and range calculations.
- Rebuilt the native and web export cards around a bright cream, purple, and yellow palette with safe edge insets, adaptive title/value sizing, wrapped localized labels and footer copy, and inset borders so neither the heading nor the top and bottom edges can be clipped in the generated PNG.
- Added serving-aware Swiss fast-food references for McDonald’s Switzerland, Burger King Switzerland, KFC Switzerland, and Popeyes Switzerland. Whole burgers and other single-item products default to `per serving` with the published or explicitly sourced serving weight while retaining gram entry; ordinary foods continue to default to grams. Each record preserves provider attribution, market, exact serving weight, and authored names in every offered language.
- Upgraded June’s bespoke programme to V8.4 from `June_Glute_Training_V8.4_29Aug2026_FINAL.pdf` and Constantine’s bespoke programme to V8.5 from `Constantin_Training_V8.5_30Aug2026.pdf`. Morning, official, and T25 blocks are now stable, separately selectable follow-along sessions; seed repair upgrades the bespoke protocols without replacing completed workout history.
- Verification: focused hotfix/programme contracts 71/71; final repository/web suite 687/687; production web build succeeded across 1,189 modules; all 4,733 translation-coverage assertions passed for every offered language; native unit suite 646/646; the focused premium-order and Workout Insights export UI journeys passed 2/2; `git diff --check` passed.
- Physical verification: the exact signed Debug build built, installed, and launched successfully on connected `iConstantine Main` as process 7018. Simulator verification used only `APEX Lane · iPhone 17 Pro` (`6907359A-18D1-46B0-87F1-13CED5CE1C46`); BA-Studio and Finalova lanes were not touched.

## 2026-08-30 — Date-owned phase history and Food Memory reliability hotfix

- Removed the redundant Simple Mode footer prompts `Food or Activity changed?` and `Open full schedule` from native and web.
- Corrected Transition and Main Phase Finished Workouts to request the current date instead of all-time history. Historical HealthKit imports remain intact and available on their actual dates, but the same test/import records can no longer repeat across every past and future phase page.
- Emptying an existing saved meal is now a valid edit. `Save changes & close` deletes the now-empty meal, persists the removal through the existing account-owned store boundary, and dismisses the composer on native and web.
- Added compact joined-word and bounded typo matching on-device and in the authenticated catalogue/corpus search routines. Production probes resolve `bigtsty`, `cheeseburgerroyal`, and `mcraclettte` to their intended Swiss McDonald's products.
- Added authored multilingual whole-serving records for Cheeseburger Royal (207 g), Big Tasty Single (273 g), Big Tasty Double (370 g), and McRaclette Classic (269 g). Each defaults to one serving while retaining gram entry and explicit source/confidence metadata.
- Restored corpus-supplied water as provider-reported hydration evidence and backfilled missing catalogue water with guarded, explicitly labelled difference estimates. Current production catalogue coverage is 94/94 foods with water; APEX plan placeholders remain excluded, and historical meal snapshots are changed only where their water evidence was previously null.
- Fixed migration 038's reserved CTE name. The linked project's legacy migration-history mismatch still prevents a safe `supabase db push`; migrations 038 and 039 were therefore applied directly and idempotently without rewriting migration history, and the Food Lookup edge function was redeployed.
- Verification: full repository/web suite 693/693; production web build succeeded across 1,189 modules; focused native Food Memory regressions 8/8; native aggregate 671/671 after stabilising the reverse-scroll smoke helper; exact simulator build/run succeeded on `APEX Lane · iPhone 17 Pro`; `git diff --check` passed.
- Physical verification: the exact signed Debug iPhone build and its embedded Watch app built successfully, installed on connected `iConstantine Main`, and launched without clearing account data. BA-Studio and Finalova simulator lanes were not touched.

## 2026-08-30 — Nutrition target integrity audit and Food Amount geometry hotfix

- Centralised native and web nutrition targets around one auditable policy. A valid owner-scoped measured resting-energy value (800–4,000 kcal/day) now takes precedence over the estimated BMR; an explicit clear remains cleared, legacy profile values migrate locally without leaking through remote profile payloads, and cache fallback is restricted to the same owner.
- Pinned the literal audit example: BMR 1,683 and TDEE 1,870 imply PAL 1.111; the corresponding raw Lean Recomp, Maintain, and Lean Bulk targets are 1,767, 1,870, and 2,001 kcal. Because PAL 1.111 is below APEX's supported 1.20 activity floor, the app does not falsely claim that 1,870 is a production TDEE unless it is supplied by an applicable bespoke protocol.
- Preserved bespoke protocol targets exactly: Constantine moderate is TDEE 2,550 / target 2,450 / 150 g protein / 75 g fat / 294 g carbohydrate; June moderate is TDEE 2,300 / target 2,400 / 85 g protein / 95 g fat / 301 g carbohydrate. Wearable calories do not silently alter either bespoke prescription or auto-switch its mode.
- Prevented blind exercise-calorie eat-back. Quick/PAL mode remains a stable planned target; Precise mode uses a same-owner, same-day whole-day wearable total as a single replacement for estimated activity blocks, never an addition, and falls back to the block sum when no usable wearable total exists.
- Corrected the Settings explanation: indirect calorimetry can provide measured resting energy, while DEXA body-fat data informs an estimate rather than directly measuring BMR. Authored the correction in every offered native locale and on web.
- Rebuilt the Food Amount presentation around one stateful, measured scroll subtree. The card now maintains at least 16 pt safe-area gutters, caps itself to available height, keeps flexible macro/input columns, hands search focus off before presentation, supports keyboard scrolling without resetting quantity, and applies the same bounded geometry to barcode results.
- Verification: full web suite 698/698; production web build succeeded across 1,189 modules; full native unit suite 667/667; focused Food Amount UI flow 1/1 on `APEX Lane · iPhone 17 Pro`; 19 native string tables parsed; `git diff --check` passed.
- Physical verification: the exact signed Debug build compiled for and installed/launched on connected `iConstantine Main`; its embedded `APEX Water` build 380 is present on the paired physical Watch. The Watch remote-debug launch endpoint was unavailable, so no unsupported claim of a remotely launched Watch interaction is made. BA-Studio and Finalova lanes were not touched.

## 2026-08-30 — Fitness Brain scientific and systems audit

- Audited the native and web Fitness Brain from profile creation through energy targets, deterministic replay, HealthKit inputs, avatar outputs, persistence, and parity tests; recorded the full findings and source ledger under `docs/superpowers/audits/`.
- Confirmed the architecture's real strengths: account-owned event history, deterministic replay, cross-platform parity, wearable deduplication, and explainable synergy receipts.
- Identified a release-blocking ordinary-account identity collision: the schema defaults a newly created profile to `constantine`, while native profile creation does not supply a generic persona. This can bypass anthropometric calorie math, select Constantine's fixed protocol, and make bespoke seed repair eligible to overwrite ordinary subscriber facts.
- Identified a second release blocker: onboarding does not collect body fat, but the schema inserts 23% and the calorie engines treat it as a credible measurement when choosing the lean-mass equation.
...
## 2026-08-30 — Fitness Brain P0 Task 3: native profile enforcement

- Added the native fail-closed profile policy with the same exact protected UUID, persona, and protocol triples used by the database and web app. Both native nutrition target paths now require that authorization before selecting a Constantine or June calorie table.
- Made native body fat nullable and provenance-bearing. Unknown, self-estimated, and legacy-unverified values cannot select the lean-mass equation; DEXA, BIA scale, caliper, and professional-estimate values may. Valid account-owned measured resting energy remains the highest-priority BMR input.
...
## 2026-08-30 — Fitness Brain v2 Project 3: immutable evidence storage

- Added the account-owned `fitness_evidence` ledger with immutable observations, source/protocol/device provenance, measured and recorded timestamps, confidence, metadata, idempotency keys, and same-owner/same-metric correction lineage. Evidence corrections append a successor instead of rewriting history.
- Enforced fail-closed metric/unit/range and source-confidence contracts in PostgreSQL. Authenticated clients have owner-scoped read access only; the security-definer recording RPC derives the owner from `auth.uid()`, admits only structured self-report or user-entered external results, and caps either at low confidence.
- Added conservative, idempotent legacy backfills for profile body-fat evidence, a user-entered external BMR result, and supported-device health metrics. Production contains `39` normalized facts: `5` low-confidence legacy body-fat values, `1` low-confidence user-entered BMR result, and `33` medium-confidence supported-device body-mass, resting-heart-rate, and VO2-max readings. No backfill was promoted beyond its permitted confidence.
- Added shared web/native source normalization and parity fixtures, account-scoped pagination and realtime merging, durable RPC outboxes, permanent wrong-owner rejection, and explicit exclusion of the evidence ledger from generic mutable-list synchronization.
- Applied migration `041_fitness_evidence.sql` directly to the linked production database. Live catalog verification found the table, `13` constraints, one owner-isolation RLS policy, authenticated `SELECT` without direct `INSERT`, and one authenticated executable recording RPC.
- PostgreSQL integration passed twice against isolated temporary clusters, including repeatable migration, deterministic backfill, RLS isolation, privilege boundaries, RPC idempotency, source spoof rejection, metadata collision rejection, same-metric correction lineage, and cross-metric lineage rejection.
- Full verification passed: web `739/739`, native unit `682/682`, native UI `25/25` in `935.147s`, focused evidence checks `4/4`, TypeScript, production Vite build, Xcode project validation, and `git diff --check`. The UI suite also now scrolls a clipped disclosure header fully clear of pinned date controls before tapping, eliminating a false-hittable SwiftUI test interaction without changing production UI.
- Built the exact signed iPhone/Watch product with Apple Development signing, installed build `380` non-destructively on `iConstantine Main`, launched `ch.apexperformance.APEX`, and confirmed the paired `Constantin’s Apple Watch` still has the embedded `APEX Water` companion build `380`.
- Next: Project 4 builds the concise, enjoyable onboarding assessment on this evidence layer; Project 5 adds the compact Avatar **Edit** control and its full **Calibrate my baseline** sheet for standard and bespoke accounts.

## 2026-08-30 — Fitness Brain v2 Project 4: distilled first-run baseline

- Replaced the cold, uniform starting-state assumption with an eight-step onboarding flow that remains short and approachable: consent, personal context, goal, normal activity week, a four-domain movement pulse, account setup, safety screening, and a confidence-aware starting map.
- Added one deterministic cross-platform baseline engine for cardiorespiratory fitness, upper-body strength, lower-body strength, and mobility. Self-report produces broad low-confidence bands only; it never fabricates a numeric Overall score, diagnostic conclusion, or high-confidence measurement.
- Persisted the resulting observations into the immutable, account-owned evidence ledger only after profile ownership exists. Each observation carries provenance, model version, measured time, confidence, and an idempotency key; bespoke programme authority remains completely separate.
- Safety answers now gate exercise clearance without turning the questionnaire into medical diagnosis. Acute symptoms stop training guidance; medical-review answers route to professional clearance; absent evidence remains unknown instead of becoming a neutral score.
- Authored all new full and constrained copy in every offered language, added domain-specific accessibility identifiers, and raised the compact-table contract to `58` entries per locale.
- Verification passed on `APEX Lane · iPhone 17 Pro`: web `745/745`, native unit `686/686`, and native UI `26/26` after an isolated rerun confirmed and a bounded scroll assertion was hardened. TypeScript, the production Vite build, all localization audits, all locale property lists, the Xcode project, and `git diff --check` passed.
- Built and signed build `381` with its embedded `APEX Water.app` and complication extension, installed it non-destructively on `iConstantine Main`, launched it successfully, and confirmed build `381` on the phone. The paired Watch still has working build `380`; installing the unchanged build `381` companion is temporarily blocked because the Watch deployment service reports the device locked or unavailable, so no Watch app or data was removed.
- Next: Project 5 adds the compact Avatar **Edit** control and resumable **Calibrate my baseline** sheet for both standard and bespoke accounts. Calibration may refine evidence but may never authorize or replace a bespoke programme or nutrition protocol.

## 2026-08-31 — Fitness Brain v2 Project 5: Avatar baseline calibration

- Added the compact **Edit** control directly above Avatar Stats on native and web, with the explicit accessibility name **Calibrate my baseline**. It opens an adaptive, resumable calibration sheet without making the required signup questionnaire longer.
- Added three honest refinement routes: a twelve-anchor observable-question journey across cardio, upper-body, lower-body, and mobility; Apple Health connection; and recent external-result entry for body fat, measured resting metabolism, VO2 max, resting heart rate, or waist circumference.
- Calibration answers produce broad low-confidence domain bands only. Sparse input remains unknown, conflicting answers widen uncertainty, and no route fabricates an Overall score. User-entered DEXA, laboratory, or device results remain low-confidence external evidence until independently supported.
- Drafts are owner-scoped and resume safely across dismissal. Saving appends idempotent observations to the immutable evidence ledger; neither a standard nor bespoke account can use calibration to authorize or replace a programme or nutrition protocol.
- Authored all new full and constrained copy in every offered language, including the final value and physiological-metric labels. The compact-table contract now requires `66` authored entries per locale, and every strings table passes property-list validation.
- Cross-platform fixture parity covers unknown domains, mixed/disagreeing answers, consistent strong signals, and invalid input. Focused web calibration verification passed `7/7`; the full web suite passed `752/752`, TypeScript and the production Vite build succeeded, and the runtime localization policy passed `4/4` with `4,848/4,848` source strings represented in every offered native locale.
- Native verification passed on `APEX Lane · iPhone 17 Pro` only: unit `690/690` and UI `27/27`. The UI flow proves the Edit control sits above Stats, opens with the correct accessibility title, persists progress after dismissal, and resumes on the saved domain. A calendar regression exposed by the month boundary was corrected to choose the first enabled adjacent empty day rather than a disabled next-month cell.
- Visually inspected the exported calibration screenshot: the adaptive sheet, title, guidance, answer cards, and controls wrap without clipping. `git diff --check` passed; BA-Studio and Finalova lanes were not touched.
- Built and signed iPhone build `382` with the embedded `APEX Water.app` also at build `382`; the Watch asset catalogue is present and embedded-binary validation passed. Installed the exact bundle non-destructively on `iConstantine Main` and launched `ch.apexperformance.APEX` successfully. No Watch source changed in this project and no separate Watch deployment was attempted.
- Next: Project 6 adds shadow validation, invariant telemetry, and rollout evidence without replacing the currently visible v1 Avatar presentation.

## 2026-08-31 — Fitness Brain v2 Project 6: privacy-safe shadow validation

- Added a shared native/web shadow-validation contract that compares the visible v1 snapshot with v2 without allowing v2 to alter the Avatar, nutrition targets, programmes, or protected bespoke authority. Only privacy-safe bands, counts, coverage, confidence, and bounded issue codes leave the device; raw measurements, evidence identifiers, names, and birthdates do not.
- Enforced cross-domain invariants: missing evidence, readiness, adherence, adaptation support, and health context cannot rewrite capacity; generic flexibility cannot become region-specific mobility; confidence remains bounded; and only owner-scoped, non-superseded evidence plus the latest owned v1 snapshot at or before the observation date are eligible.
- Added an owner-derived, RLS-protected PostgreSQL observation ledger, an authenticated security-definer recording RPC, and a service-role-only aggregate review RPC. Daily native/web outboxes are durable and last-write-wins for pending observations while preserving in-flight work and account boundaries.
- Added an explicit rollout gate and claim ledger. V2 remains `shadow_only`: activation requires at least 1,000 observations, at least 100 per reviewed subgroup, at least 0.80 coverage, no more than 0.05 outliers, zero invariant failures, completed scientific/privacy/accessibility reviews, and separate owner approval. The system cannot self-activate.
- Corrected visible Avatar language to **Training estimate** and **Legacy game scores. Not medical measurements.** The claim audit permits training estimates, confidence, coverage, provenance, and uncertainty while rejecting diagnosis, injury prediction, guaranteed outcomes, or claims of exact physiological accuracy.
- Applied `042_fitness_brain_shadow_validation.sql` directly and idempotently to the linked production database without rewriting its legacy migration history. Live catalog verification found the table, `17` constraints, one owner-select policy, authenticated read without direct insert, the authenticated recording RPC, and the service-only review aggregate. The table starts empty by design and collects only after compatible clients run.
- Isolated PostgreSQL 15 integration proved repeatable migration, same-day idempotent upsert, owner RLS, malformed/identifier-shaped payload rejection, and identifier-free aggregation. Focused shadow, database, synchronization, and claim verification passed `40/40`; claim-policy checks passed `3/3`; localization coverage passed `4,850/4,850` across every offered language.
- Full verification passed: web `770/770`, production Vite build, and native unit `697/697`. The full native UI run passed `26/27`; its only failure exposed a debug-only shadow replay assertion during the first-run fixture. After adding an explicit UI-test boundary, that entire onboarding/plan-rebuild regression passed `1/1` in `70.706s`, so every one of the 27 UI journeys has current passing evidence. All simulator work used only `APEX Lane · iPhone 17 Pro` (`6907359A-18D1-46B0-87F1-13CED5CE1C46`); BA-Studio and Finalova lanes were not touched.
- Built and strictly verified signed iPhone build `383` with its embedded `APEX Water.app` also at build `383`. Installed the exact bundle non-destructively on `iConstantine Main` and launched `ch.apexperformance.APEX` successfully; no Watch source changed and no separate Watch deployment was attempted.

## 2026-08-31 — Baseline calibration clarity and DEXA feedback hotfix

- Replaced the long-form calibration's compound three-prompt pages and generic repeated picker labels with twelve direct, one-question screens. Every prompt now has four plain-language, question-specific choices plus an explicit **I'm not sure or haven't done this recently** option; **Continue** remains disabled until the user deliberately answers.
- Preserved private owner-scoped progress while adding migration for earlier drafts, so an interrupted calibration resumes at the first unanswered question instead of silently treating untouched defaults as answers.
- Rebuilt the native and web calibration entry surfaces with a brighter evidence-first hierarchy, clearer time expectations, large accessible answer cards, review feedback, Reduce Motion support, and broad low-confidence bands that still cannot create an Overall score or rewrite any training or nutrition plan.
- Split recent evidence into a source-first chooser. A DEXA report can now save body-fat percentage and the report-supplied BMR/RMR estimate together, while clearly stating that DEXA measures body composition and that a printed resting-energy number is report-supplied rather than a direct DEXA metabolic measurement.
- Added persistent, unmistakable save confirmation listing the normalized values. Moved native sheet presentation to the stable portal shell so Avatar recomputation after an evidence write cannot dismiss the confirmation. The captured passing flow visibly retained **18.4%** and **1’683 kcal/day** with no clipping or truncation.
- Authored all new full and constrained strings in every offered language. The compact-table contract now requires `69` entries per locale, all native property lists parse, and the runtime corpus reports `4,938/4,938` represented strings in every offered native locale.
- Verification passed: focused web calibration/localization `15/15`, complete web `774/774`, production TypeScript/Vite build, focused native calibration/localization/UI `7/7`, complete native unit `700/700`, strict signing, and `git diff --check`. Simulator verification used only `APEX Lane · iPhone 17 Pro` (`6907359A-18D1-46B0-87F1-13CED5CE1C46`); BA-Studio and Finalova lanes were not touched.
- Advanced the release monotonically to build `384` for both iPhone and embedded `APEX Water.app`. Installed the exact signed bundle non-destructively on `iConstantine Main`, confirmed installed version `1.0.0 (384)`, and launched `ch.apexperformance.APEX` successfully. No Watch source changed and no separate Watch deployment was required for this iPhone calibration hotfix.

## 2026-08-31 — Visible Avatar calibration replay hotfix

- Diagnosed the saved-but-unchanged Constantine report at the actual runtime boundary: the questionnaire correctly appended account-owned `fitness_evidence`, but the visible v1 Avatar replay never received that ledger. Evidence reached only the shadow validator, so every displayed lane remained on its persona seed and ordinary history.
- Added one narrow native/web calibration bridge for the two authored structured-self-report protocols. Cardiorespiratory, upper-strength, lower-strength, and flexibility evidence now anchors only its matching visible capacity lane on the evidence date; Overall remains a derived composite, while Health, Joint, nutrition targets, and standard or bespoke programme authority are untouched.
- Kept self-report conservative. Its influence is weighted by the existing evidence-coverage rule and capped at `0.55`; missing input remains missing, foreign owners and future rows are rejected, superseded rows are ignored, and repeated same-day questionnaires use only the newest observation instead of stacking scores.
- Made successful offline questionnaire completion locally effective as well as durable: a transient/authentication retry now merges the normalized account-owned record into the local ledger before saving the snapshot, while the same idempotency key later lets the authoritative RPC/realtime row replace it safely.
- Red/green evidence reproduced all four unchanged Constantine seed lanes before the fix, then proved exact visible movement afterward. Focused verification passed web `50/50` and native `26/26`; complete verification passed web `775/775`, native unit `701/701`, TypeScript, the production Vite build, strict code-sign validation, and `git diff --check`.
- Advanced both iPhone and embedded `APEX Water.app` monotonically to build `385`. Installed the exact signed bundle non-destructively on `iConstantine Main`, launched `ch.apexperformance.APEX` successfully with PID `4626`, and preserved the authenticated account data. Simulator work used only `APEX Lane · iPhone 17 Pro`; BA-Studio, BA-Studio-Small, Airlore, and Finalova lanes were not touched.

## 2026-09-01 — Intent-aware Food Memory search hotfix

- Replaced permissive substring search with one shared intent-aware ranking contract across web, native, the canonical corpus RPC, and the deployed Food Lookup function. Every meaningful query token must now align, exact and prefix matches lead, adjacent fragments such as `ext;ra` can rejoin, bounded misspellings such as `vlrgn` resolve, and unrelated partial matches can no longer leak into `extra virgin` results.
- Added food-category priority for a one-word oil search: extra-virgin olive oil leads, then olive and vegetable cooking oils, then other pure oils, while margarine is explicitly demoted. Local favourites and usage history remain useful tie-breakers but cannot outrank query relevance, and merged local/remote results are globally reranked before display.
- Added authored multilingual records and aliases for wild broccoli / broccoli rabe / rapini and broccolini / thin-stem broccoli. The records preserve their distinct USDA FoodData Central and Australian Food Composition Database provenance, per-100-g basis, water, macros, fibre, and source identifiers without deriving missing nutrients.
- Applied idempotent production migration `043_food_search_relevance.sql` directly to the linked Supabase project and deployed `food-lookup` version `7`. Live RPC proof showed `Oil` led with extra-virgin olive oil, `ext;ra vlrgn` and `extra virgin` returned only relevant oil rows, both broccoli intents resolved, and the migration retained strict owner/security boundaries.
- Verification passed: complete web `779/779`, focused search/localization `6/6`, production Vite build, native food/localization `66/66`, current-code native Food Memory parity `9/9`, strict signed-bundle validation, and `git diff --check`. All simulator work used only `APEX Lane · iPhone 17 Pro` (`6907359A-18D1-46B0-87F1-13CED5CE1C46`); BA-Studio and Finalova lanes were not touched.
- Advanced the iPhone and embedded Watch product monotonically to build `386`, built and strictly verified the signed bundle, installed it non-destructively on `iConstantine Main`, and launched `ch.apexperformance.APEX` successfully after the phone was unlocked. Installation preserved the authenticated account data.

## 2026-09-01 — Hydration complication stale-zero source hotfix

- Reproduced the stuck-zero complication at the canonical-resolution boundary: a delayed HealthKit sidecar deletion with the same base revision could erase a newer account-owned companion snapshot, turning a valid `1,250 mL / 3,000 mL` state into `0%` even though the Watch app and phone still showed the logged water.
- Made the account-owned companion snapshot authoritative for removals. A sidecar may still surface a newly observed external HealthKit addition early, but it can no longer delete canonical APEX hydration; deletion reconciliation remains owned by the Watch app and advances the canonical causal revision.
- Tightened day rollover. The explicit midnight zero still clears yesterday from the face, and the timeline now requests a fresh canonical entry one minute after midnight rather than permitting that zero to remain until a delayed half-hour refresh.
- Red/green regression evidence covers both failure paths. The focused pair failed with canonical hydration incorrectly resolving to `0 mL` and refresh scheduled for `00:20`, then passed after the fix. The complete `WatchHydrationFillStateTests` suite passed `50/50` on `APEX Lane · iPhone 17 Pro`; `git diff --check` passed.
- An unsigned generic watchOS Release build succeeded, including embedded `APEX Water.app` and complication-extension validation. At the owner's explicit overnight request, no physical iPhone or Apple Watch was contacted, installed, launched, or woken, and the build number was not advanced. Exact signed-device installation plus face-value/add/remove verification remains intentionally deferred until the owner confirms they are awake.

## 2026-09-01 — Nutrient evidence and observed-intake patterns

- Added a bounded nutrient-evidence contract across provider import, the canonical food corpus, private food records, offline synchronization, and immutable logged-food snapshots. Reported values retain their source unit, original value, basis, provenance, and status; trace, below-detection, not-measured, and unavailable observations remain distinct and are never converted into zero. Immutable receipts now preserve coarse sugar, fibre, and saturated-fat facts beside extended evidence instead of silently dropping either source.
- Added an accessible circled information control to the native and web food-amount flows used after typed search or barcode scanning. Its optional detail sheet groups vitamins, minerals, fat details, carbohydrate details, and other nutrition while preserving the intentionally concise default amount screen. It discloses source, reference, original source value, normalization, and a branded-product label warning; the web nested dialog traps focus, closes with Escape, restores focus and honours Reduce Motion. A food with no detailed source evidence says so honestly instead of fabricating a nutrition table.
- Added a private **Observed intake** board to Nutrition with Day, 7-day, and month-to-date views. It reports portion-scaled averages only across days containing logged food, discloses observed days beside calendar days, evidence coverage, and detailed-food count, keeps incompatible units separate, and uses relative bars without diagnosing deficiency, excess, or an individual's biological status. The concise view keeps the strongest three observations per nutrient category, while an accessible control exposes the complete bounded set without a hidden first-16 cap.
- Added detailed Open Food Facts mapping for mono- and polyunsaturated fat, trans fat, cholesterol, omega-3/6, added sugars, starch, sodium, the supported vitamin family, and major minerals. Provider modifiers now preserve `~` as trace, `<`/`≤` as below detection, and `-` as missing with the original source text; an ordinary reported zero remains a real zero. The edge lookup returns the bounded evidence payload while corpus rows without source detail remain explicitly unavailable.
- Tightened `044_nutrient_evidence_and_patterns.sql` to reject unknown keys, oversized JSON, more than 96 observations, overlong strings, out-of-range values, and numeric values attached to missing/trace states. Corpus evidence and backfill now require both an active registered source and its active validated batch. Live transaction-only fixtures proved staging and retired batches remain hidden while the same active batch is exposed; live validator checks accepted a valid reported value and rejected an unknown key, numeric trace, and 97-row payload.
- Applied migration `044` directly and idempotently to the linked production database without rewriting its divergent legacy migration history. Reworked the existing food-corpus search RPC to prefilter through its trigram GIN index before fuzzy scoring; live V3 search returned four bounded valid `extra virgin` results. Deployed `food-lookup` version `9`, including the corrected source-modifier mapper, and confirmed it `ACTIVE`.
- Authored the new full interface copy, provenance guidance, expansion controls, and nutrient names in every offered language. All eight non-English native string tables parse, runtime localization and layout policies pass, and no Dynamic Type scale-down or truncation workaround was introduced.
- Verification passed: complete web `787/787`, complete native unit `706/706`, focused native parity/evidence `12/12`, focused amount/detail and nutrient-pattern UI journeys `2/2`, TypeScript, production Vite build, native simulator build, executed migration gates, live RPC proof, and `git diff --check`. Visual verification used only `APEX Lane · iPhone 17 Pro` (`6907359A-18D1-46B0-87F1-13CED5CE1C46`); BA-Studio and Finalova lanes were not touched. Per the owner's overnight request, no physical iPhone or Apple Watch was contacted, installed, launched, or woken.

## 2026-09-01 — Coach platform foundation

- Added a server-authoritative coach workspace with private email-bound invitations, explicit client-selected sharing scopes, separate opt-in for visual progress, searchable roster attention, consent-filtered client overviews, immutable versioned plans, client acknowledgement, atomic activation, relationship ending, and an append-only audit trail. Direct table access is revoked; authenticated workflows derive the actor from `auth.uid()` and enforce account ownership at every RPC boundary.
- Added sponsored-client policy without weakening individual ownership. A sponsored client keeps Nutrition, the coach-authored Fitness Plan, and Avatar access; coach visibility is limited to the scopes the client actively shares. Custom plan creation, rebuild, and Orbit remain unavailable under sponsorship alone, while separately purchased individual access survives sponsorship or grace.
- Added matching native and web coach experiences for invitation review, consent, client roster, reviewed plan authoring, safe draft/version conflict handling, publication, acknowledgement, activation, privacy changes, and relationship closure. Development coach access remains server-gated; no coach relationship or client access is inferred from a display persona.
- Applied production migrations `045_coach_platform_foundation.sql` and `046_coach_overview_current_plan.sql` as linked timestamps `20260901120000` and `20260901130000`. The follow-up returns the current immutable version to the coach, keeps invitation previews bound to the authenticated email, and exposes only the consented client context.
...
## 2026-09-01 — Detailed nutrition hierarchy hotfix

- Replaced the fragmented nutrient-category cards with one familiar, readable **Nutrition facts** hierarchy on native and web. Calories lead; **Total fat** owns saturated, trans, mono- and polyunsaturated, and omega details; **Total carbs** owns fibre, sugars, added sugars, and starch; cholesterol, sodium, protein, water, vitamins, and minerals follow without duplicating totals in an “Other nutrition” bucket.
- Removed the blood-drop treatment from fat and removed the repeated status, original-value, source, and raw-reference boilerplate from every row. The optional sheet now shows the food, per-100 basis, nutrient name, and normalized value only. The data engine still preserves the complete source evidence internally, while one natural Food and nutrition data paragraph in Terms explains provenance, variable coverage, trace handling, and checking a current package label.
- Added a clear parent/detail typographic hierarchy with indented sub-nutrients, adaptive wrapping, concise non-trailing-zero number formatting, and separate restrained Vitamins and Minerals sections. Unknown preparation states no longer leak into the heading.
- Authored **Nutrition facts**, **Per 100**, **Total fat**, **Total carbs**, and the Terms disclosure across every offered native language, with corresponding web copy where the runtime translation layer requires it. Runtime localization coverage is `5,112/5,112` in all nine locales.
- Added shared native/web ordering contracts plus native UI assertions for total/detail attachment, indentation, and absence of the removed provenance lines and raw references. The dedicated simulator journey passed `1/1` on `APEX Lane · iPhone 17 Pro`; BA-Studio and Finalova lanes were not touched.
- Verification passed: complete web `803/803`, complete native unit `721/721`, localization policy `4/4`, production TypeScript/Vite build, native detailed-nutrition UI `1/1`, all native string tables parsed, and `git diff --check`. Prepared monotonically advanced iPhone and embedded Watch build `387` for signed-device installation.

## 2026-09-02 — Natural-food micronutrient evidence release

- Added an exact, reviewed natural-food evidence bundle covering 111 foods across nine categories with 4,254 nutrient observations. The pipeline preserves source units, trace observations, missing values, preparation state, and provenance; it rejects ambiguous matches rather than inventing nutrients.
- Added deterministic source-specific resolution for all 155 reviewed USDA SR Legacy, USDA Foundation, and Frida nutrient identities. Vitamin A keeps its source semantics (`µg RAE` for USDA and `µg RE` for Frida), alternate forms cannot double-count, unknown source/code pairs remain unchanged, and arachidonic acid is restricted to its reviewed SR Legacy identity rather than misclassifying Foundation 20:3 n-6.
- Added migrations 048 and 049 for the reviewed food evidence and server-side nutrient/unit hardening. Migration 049 is idempotent, applies the same projection to future corpus writes, and revokes direct execution of its internal helpers. Local explicit evidence remains highest priority, followed by exact server evidence and then the bundled fallback; private foods and observed averages remain account-scoped.
- Added native and web runtime hydration for vitamins, minerals, fatty-acid details, carbohydrates, water, and related evidence. Detailed Nutrition now keeps total fat and total carbohydrates attached to their children, presents Vitamins and Minerals as readable sections without developer provenance clutter, and retains missing-versus-zero semantics.
- Added daily, weekly, and monthly observed nutrient averages based only on reportable logged evidence, with portion scaling, unit isolation, local-date windows, and per-nutrient coverage instead of diagnosing deficiency.
- Added authored user-facing copy in every offered language plus Settings food-data acknowledgements. Scientific unit abbreviations remain exact and are covered by the localization audit.
- Hardened the food amount popover against negative, non-finite, and undersized geometry. The final APEX-lane UI journey verified search, the amount sheet, circled nutrient information, fat/carbohydrate hierarchy, vitamins, minerals, decimal-keyboard dismissal, and successful food addition without clipping or a stuck presentation.
- Deterministic artifact hashes: `shared/natural-food-evidence.json` = `6de29d97486ed8fccc0407b624bd9002bcb16ff30fd90b169821eeece2b44a50`; migration 048 = `b8aba1c593d8ad2be9430d668f69ce3ca44feb649db404316561e6c5ed6442a6`.
- Final verification passed: full web/data suite `867/867`; affected native release suite `95/95`; real UI flow `1/1`; production web build; migration 049 exercised twice against real PostgreSQL; `git diff --check`; 111 foods / 4,254 observations / 15 reviewed SR Legacy arachidonic-acid rows / zero Foundation misidentifications.
- The bundled evidence is available without a remote database rollout. Production Supabase migrations were not applied from this non-interactive session because the production credential remains unavailable; this is recorded rather than silently claiming a server deployment.
- No physical iPhone or Watch install was performed during this release verification because the owner asked that sleeping devices not be woken. Simulator verification used only `APEX Lane · iPhone 17 Pro`, avoiding the BA-Studio and Finalova lanes.

## 2026-09-02 — iOS client reliability audit baseline

- Established a 35-case end-to-end acceptance matrix spanning fresh authentication, legal and entitlement exits, onboarding atomicity, goal math, nutrition and food privacy, hydration, workout authoring and receipts, wearable association, Avatar propagation, calibration, recovery planning, visual progress, Orbit, account switching, coach publication, offline recovery, permissions, accessibility, and true unseeded launch.
- Every case now names its persona, active owner, selected date, action and destination, persistence/downstream assertion, negative branch, automated artifact, and—where applicable—human discoverability script. Unproven paths remain explicitly `Pending`; passing fixture smoke tests are not presented as complete user-flow proof.
- Recorded five static navigation/lifecycle defects for their owning repair tasks: saved custom workouts cannot reopen editable; restricted creation can report a false success; same-weekday creation can silently replace a workout; Orbit Home recent-run chevrons are inert; and historical Orbit debrief owns a nested navigation stack.
- The final full UI baseline on `APEX Lane · iPhone 17 Pro` passed **30/30**, with zero failures or skips in 1,165.655 seconds. Evidence is retained at `build/apex-client-audit/APEXUI-task1-final.xcresult`; BA-Studio and Finalova lanes were not used.
- Retained, rather than concealed, a slow repeated-scroll test-harness path, the Xcode debugger-version infrastructure warning, and Swift actor-isolation warnings. Task 1 intentionally changes no product behavior; those findings flow into the numbered implementation tasks.
- Physical iPhone and Watch installation remains deferred until the integrated audit repairs are complete, so the owner receives one exact, fully verified signed build rather than an intermediate audit baseline.

## 2026-09-02 — iOS client reliability audit: account and privacy boundaries

- Added a canonical `AccountOperationLease` carrying both owner and account generation, then required it across audited asynchronous mutations and private reads so a delayed Account-A success, failure, callback, upload, search, Health import, workout save, Orbit operation, coach action, or settings write cannot publish into Account B.
- Account boundaries now synchronously clear account-derived Avatar, navigation, coach, entitlement, Health, notification, progress-media, workout, and Orbit presentation state. Authenticated-but-profileless owners remain explicit rather than inheriting the previous profile, and debug first-run fixtures are owner-bound.
- OAuth callbacks remain on a non-writable loading route until an owner-scoped cache or server dashboard proves identity. If the server refresh then fails, APEX revalidates the cached authenticated owner, restores coach context and entitlements, and restarts realtime; an unprovable cache returns to clean sign-in.
- Health import is now an account-scoped opt-in with owner-qualified observer callbacks, workout import state, and dietary-water anchors. Denial or stale completion cannot replace another account's visible state.
- Watch workout handoff now assigns one durable UUID from iPhone launch through Watch start and stop. The Watch persists cancellation/disconnect state, queues an early HealthKit configuration until its owner-qualified command arrives, rejects delayed older launches, and prevents an old stop or disconnect from ending or clearing a newer same-owner or Account-B workout.
- Nudge read state and local-notification identifiers are owner-qualified. Logout and A → B transitions synchronously remove the old owner's known notification requests, while a guarded asynchronous inventory clears legacy identifiers without racing a new owner.
- Private visual progress now revokes camera authorization/startup, capture callbacks, photo preparation, signed URLs, comparison reads, and uploads on dismissal or account change. Custom Close and system/interactive dismissal both stop the camera and invalidate the capture request.
- Independent adversarial review found and closed four final lifecycle classes that the earlier green suite did not behaviorally exercise: late Watch launch after stop, cached OAuth services left dormant, imminent old-owner notification delivery, and camera publication after dismissal. Follow-up review also closed delayed disconnect and direct canonical A → B Watch-owner races.
- Final verification on `APEX Lane · iPhone 17 Pro`: **351/351** relevant native tests passed with zero failures or skips at `/tmp/APEXTask2-combined-green-final-5.xcresult`; the focused late-race gate passed **107/107** at `/tmp/APEXTask2-final-races-green-20260902-2.xcresult`; all 60 changed Swift files parsed; generic compiler-only build-for-testing succeeded; and `git diff --check` passed. BA-Studio and Finalova simulator lanes were not used.
- Physical iPhone and Watch installation remains intentionally deferred until the complete audit release is integrated; this task does not claim device-level workflow or human-discoverability evidence.

## 2026-09-03 — iOS client reliability audit: goal and energy math

- Completed the goal and energy-math audit across native and web. Generic Lean Recomp, Maintain, and Lean Bulk now seed at 90%, 100%, and 105% of habitual TDEE, while observed wearable activity remains informational and never triggers automatic calorie eat-back.
- Made valid indirect-calorimetry resting energy authoritative. DEXA-reported RMR remains lower-confidence estimate context and evidence, and cannot overwrite a stronger measurement.
- Added explicit target provenance, review states, and fail-closed gates for malformed or non-finite inputs, unsupported resting energy, infeasible macros, and meal-plan publication. Exact authored bespoke calorie and macro tables remain preserved and reviewable rather than being silently recalculated.
- Unsupported activity values now fail safely, user-facing activity targets accept only the active owner's rows on the selected date, and blocked or otherwise unpublishable targets render review guidance instead of numeric zero prescriptions on native and web.
- Kept profile, settings, activity, evidence, targets, and persistence account-isolated. These values are transparent estimates with visible review paths; APEX does not claim medical certainty.
- Verification passed: complete web `913/913`, complete native unit `884/884`, focused owner isolation `2/2`, onboarding, DEXA, and goal-information UI `3/3`, production web build, localization coverage, and `git diff --check`. Simulator verification used only the dedicated `APEX Lane · iPhone 17 Pro` (`6907359A-18D1-46B0-87F1-13CED5CE1C46`).

## 2026-09-03 — iOS client reliability audit: account access and onboarding truth

- Added the server-authoritative TestFlight entitlement contract through migration `050_testflight_account_entitlements.sql`. Production now has entitlement rows for all `6/6` accounts: the four protected accounts retain permanent access, while the two ordinary accounts receive the fixed TestFlight expiry of `2027-12-31`. Row-level security and a security-definer RPC enforce owner-derived access; authenticated and anonymous clients have no direct access to the entitlement tables.
- Rebuilt native and web access resolution around fail-closed, owner-scoped caches. Finite and sponsored grants require a continuous boot session plus monotonic expiry, launch and foreground revalidate with the server, clock rollback and malformed evidence cannot extend access, and an access denial immediately stops private Health, hydration, Orbit/location, realtime, outbox, and account-derived background work.
- Added truthful recovery states for update-required, revoked, expired, locked, and unavailable access, with safe sign-out, retry, and coach-invitation acceptance. Restored access resumes private work only after the authoritative account boundary has been re-established.
- Removed native StoreKit/paywall and beta-code entry from the TestFlight path; both remain deferred to Roadmap Phase 5. Sponsored clients retain only the explicitly permitted coach-plan, Nutrition, and Avatar capabilities unless they also hold individual access; custom plan creation and rebuild, custom workouts, and Orbit remain restricted.
- Kept required onboarding concise while making its answers deliberate: required questionnaire and safety choices cannot be advanced accidentally, and declining health-data consent provides a clear setup exit/sign-out without collecting Health data.
- Verification passed: full native unit `906/906`; focused access, coach, and Health `65/65`; native UI `3/3`; complete web `954/954`; production web build; native localization `5,167/5,167` in every offered language; privacy-manifest, migration, localization, and source-policy checks. Independent native review found no remaining P0 or P1 issue.
- Physical iPhone and Apple Watch installation remains deliberately deferred to integrated audit Task 8. This task does not claim device installation or device-level verification.

## 2026-09-07 — iOS client reliability audit Task 5: policy-aware creation and editing

- Applied coach-sponsored capability checks consistently across Simple, Advanced, Settings, Avatar, nutrition target editing, and workout creation. Retained personal induction data can no longer override sponsored selection or bypass the direct workout-player guard; individual access retains personal features.
- Completed the custom-workout lifecycle on native and web: create, reopen with the saved prescription, edit, explicitly confirm exact same-weekday replacements, and confirm removal from the active plan. Fresh replacements receive new day identities and archive old days rather than deleting historical receipts or their prescriptions. Explicit edits preserve the day identity; existing logged exercise names/values survive exercise-row replacement.
- Made custom save/removal success follow durable persistence. Web stages from current owner data and journals cache plus ordered outbox before publication under a synchronous shared mutation lease. Native stabilizes the owner cache before enqueue, guards rollback across account boundaries, and treats durable enqueue as the commit point; a later cache-rebase error cannot report already-queued work as unsaved. Parent/day/new-exercise writes precede stale exercise deletes.
- Emptying an existing meal now presents explicit confirmed deletion. Native deletion checks the exact owner, date, meal and entries, persists before publishing, and narrowly restores the affected meal if neither remote nor outbox persistence succeeds. Unrelated nutrition changes survive asynchronous work. Web uses its existing durable meal transaction after owner/date checks and explicit confirmation.
- Authored native full/compact copy in all nine offered languages and web copy in its three offered languages (English, Romanian, Thai). Native runtime coverage: `5,178/5,178` per language.
- Verification: pre-review native full suite `931/931`; after the three concrete safety-review fixes, only the four affected suites were rerun, passing `52` test functions / `61` executions, zero failures/skips (`build/APEX-Lane-Task5-integrated/NativeFixGreen.xcresult`). Focused web lifecycle/persistence/sync `43/43`, complete web suite exit `0`, production TypeScript/Vite build passed (`1,213` modules), and diff whitespace checks passed. Existing bundle-size warnings remain unchanged. Independent task review and scoped fix review resolved the identified owner, queue and sponsored-route findings; no additional full native or UI rerun was performed, per the owner's request to avoid unnecessary checks.
- The custom-workout UI create/reopen/edit/replace/delete journey previously passed `1/1` on the APEX simulator lane; its cached result bundle is no longer present, so this remains historical evidence rather than a newly inspected artifact. Final integrated UI/device verification remains Task 8. On September 5, during the pause and at the owner's separate request, signed iPhone build `388` was installed over the existing app without deleting data; it predates today's final safety fixes. No new physical-device installation is claimed here.
- Remaining audit sequence: Task 6 (dates, attribution and deletion), Task 7 (targeted functional inconsistencies), then Task 8 (bounded integrated verification/publication and signed iPhone/Watch delivery). Known durability limit: if a post-commit cache refresh fails, the committed workout remains in the last durable snapshot/outbox; unrelated concurrent fields continue through their normal persistence path.

## 2026-09-07 — iOS client reliability audit Task 6: completion dates and generated activity

- Task 5 was published as `72f6405fa3ac046dd8d07d8af6159d1255703d25` to both required refs. GitHub Pages run `34162564950` succeeded for that exact commit; live APEX returned HTTP 200.
- Native guided and tracked players now carry the calendar-selected date through presentation and completion. The existing completion boundary then applies that date to the receipt, deload calculation and generated activity rather than defaulting historical sessions to today. Web already carried the selected date and retains that behavior.
- Native workout-generated activity now uses the existing cross-client deterministic owner/date/receipt identity. Native and web deletion plans remove only matching owner-owned `workout_module` activity, including the existing Focus T25 secondary contribution, alongside the receipt's own set rows. Unlinked wearable records retain their original Apple Health UUID and energy; unrelated activity is preserved.
- Verification was intentionally bounded: two targeted web failures reproduced before repair; four relevant web cases passed afterward; exactly `3/3` native receipt/date/owner tests passed (`build/APEX-Lane-Task5-integrated/Task6TargetedGreen.xcresult`); production web build and diff checks passed. Root independently reviewed the changed date/deletion paths and native/web ID algorithms. No repeated full native/web/UI suite, physical workout logging, new schema or new user-facing copy.
- Limitation: old native generated activity with random IDs and no durable receipt link cannot be identified safely. It is preserved rather than deleted using date/name guesses. Newly generated activity and already deterministic web records have exact deletion attribution. Next: Task 7's focused functional repairs, then Task 8 delivery.

## 2026-09-07 — iOS client reliability audit Task 7: functional consistency

- Task 6 was published as `329e02e2bdfd3ee6a0ee57c42e4c53c7123d0e22` to both required refs; Pages run `34163865775` succeeded for that exact commit and the live entry bundle was confirmed.
- Recovery replacement now excludes the same-owner future rows it will deactivate when choosing dates, while protecting recorded sessions and other recovery targets, on native and web.
- Native progress capture saves the explicitly entered weight (including decimal-comma input), rather than silently substituting profile weight. The comparison's Include stats choice now reaches the exported poster. Persisted profile avatars resolve through account-bound signed URLs, with cancellation and fallback guards.
- Native marathon minimum/full choices carry their different authored duration targets into the live run and interrupted-run draft, with pause-excluding completion guidance and account-boundary clearing. Avatar snapshot persistence now notices individual component changes even when overall score is unchanged, on both clients.
- Removed the duplicate comparison setting and the manual logger's unsupported reusable-preset promise. Orbit recent runs open their historical debrief without a nested navigation stack. Measured BMR validation was inspected and already rejects invalid submissions without replacing evidence; it was deliberately left unchanged.
- Bounded verification: recovery scheduling failure reproduced before repair, then `7/7` focused web tests passed; exactly `3/3` native tests passed with no skips (`build/APEX-Lane-Task5-integrated/Task7TargetedGreen.xcresult`). Production web build passed, and root reviewed the complete scoped source/test diff plus ownership, storage and deterministic snapshot identity paths. No broad regression rerun, new localization keys, real workout or Health mutation. Final Release compilation covers two subsequent scalar LiveRunView guards.
- Real-account avatar download and physical-device behavior are not claimed from these automated results. Integrated release compilation, essential UI smoke and signed build delivery follow in Task 8.
- Signed generic-iPhone Release compilation with `CURRENT_PROJECT_VERSION=389` subsequently passed, covering the final LiveRunView guards. Existing concurrency warnings remain; no compilation errors.

## 2026-09-08 — Bounded audit release: build 389 delivered

- Task 7 published as `1e7f49ccc1ecea2d5a6bca6fa41e01a6a2ca1834` on both required refs. Pages run `34165056911` succeeded for that exact SHA; live entry `index-mV-Rje0m.js` was verified.
- Honored the owner's explicit limit on further regression checks: reused earlier full-suite evidence and ran only two integrated UI journeys on the dedicated APEX lane. Custom-workout create/reopen/edit/replace/delete and Avatar visual-progress placement plus actionable recovery planning passed `2/2`, no skips or failures (`build/APEX-Lane-Task5-integrated/Release389UISmoke.xcresult`). These are focused journey results, not proof that every acceptance-matrix branch has been exercised.
- Signed Release build `389` passed deep strict code-signature verification. Installed over the existing iPhone app, launched successfully, and queried installed version `389`. Installed the embedded signed `APEX Water.app` on the physical Watch; the first connection timed out, and one bounded retry succeeded. No uninstall, account reset, test workout, dietary-water write or artificial Health sample was performed.
- Outstanding evidence remains explicit: real-account avatar download, visual watch-face hydration reconciliation, broad Dynamic Type/Reduce Motion permutations and the remaining human-rated walkthrough matrix are not established by this release. Old random-ID generated activity without durable attribution remains preserved. The broader calorie-calibration cadence and generated-plan time/prescription personalization findings remain open; no claim of medical certainty or exhaustive audit completion.
- Latest working web: https://evoryder8-collab.github.io/APXAppiC/ . This documentation-only release record does not change the installed binary.

## 2026-09-08 — Dayline parity and progressive Workout Insights

- Native Dayline now draws curved connections from each meal/workout's actual timestamp to its collision-separated card. Recorded connections are solid and planned meals dashed; cables are decorative, non-interactive and hidden from accessibility. Existing timing, drag, swipe, logging and energy calculations are preserved.
- Simple now places Wearable Activity above Workout Insights on native and web. Insights begins as a narrow accessible disclosure and only reveals its controls when expanded; other pages retain their expanded presentation. State is transient, resets on owner change, and respects Reduce Motion. Existing localized labels are reused.
- Focused verification: the new disclosure journey failed before implementation and passed afterward (`1/1`); the Dayline completed-workout/recovery screenshot journey passed (`1/1`) and its attachment was inspected. Native simulator build and production web build passed; existing unrelated concurrency/bundle-size warnings remain. No broad test suite or physical Health mutation.
- Developer Mode is a separate pending part of this approved design. It must provide isolated Individual, Coach and Invited-client experiences plus return to the real bespoke account; this presentation commit does not claim that selector is implemented.
- Scoped review closed a persisted web block-order edge case by grouping Finished Workouts, Wearable Activity and Insights together; its focused behaviour test passed `1/1`. Root screenshot inspection found the initial native cable was obscured by a node, so the final path reaches the card edge with a true-time pin. A close-meal (07:00/07:10) screenshot journey passed `1/1` and root confirmed the visible curved route. Scoped re-review approved both fixes, without broad test repetition.

## 2026-09-08 — Native developer previews and return to the bespoke account

- Added temporary Developer Mode immediately below Active Identity for the authenticated Constantine bespoke owner. The selector offers My account, Individual subscriber, Coach and Invited client. Release authorization still uses the existing protected-owner/profile integrity policy; the UI-test fixture gate is DEBUG-only.
- Each preview creates a new isolated AppSession, sample identity graph, private preferences suite, clientless service and nonpersisting store. It uses the real onboarding, plan generator, portal, local nutrition/workout editing, coach plan editor/publication and invited-client acknowledgment/activation flows. Returning revokes sample operation leases and discards sample data without replacing the authenticated session. Ending sample sponsorship returns to My account rather than granting an individual tier.
- Device/remote integrations are deliberately unavailable in preview: Health/Watch writes, auth, purchases, notifications, photos, Orbit tracking/route generation and barcode camera. Sample food search uses a small local catalogue. The permanent scene-local My account control stays above presentations, while nonintegration changes remain local and interactive. Authored full copy covers all nine offered languages; three constrained labels have authored compact forms.
- Focused verification: six sandbox behavioral tests passed (identity/entitlements/lease isolation, local meal and hydration CRUD, coach/client operations, individual skip/build/rebuild, no disk/outbox writes); compact-table contract passed for all nine languages; end-to-end selector → individual setup → real account → coach invitation sheet → real account passed. UI work uncovered and corrected selector discoverability and return-window geometry; final journey is `Test-APEX-2026.09.08_02-42-41-+0200.xcresult` in the retained APEX build cache. A read-only isolation review found the scanner constructor escape, now guarded before camera creation.
- This closes only the approved Developer Mode implementation alongside the already-published Dayline/Insights change (`df3b06e47e4876e604a08c5dc9abb69fb8743062`, Pages run 34172441580). It does not claim the wider historical audit or physical complication observation is complete. Release 390 delivery/publication evidence is recorded below when available.

### Release 390 delivery evidence — 2026-09-08

- Implementation commit `31bd1a08ba7f97801c36a058cbc117d33f90fd4f` pushed to both `codex/main-critical-repair` and `main`. GitHub Pages run `34174591515` succeeded for that exact SHA; live site returned `assets/index-fypxJ-2x.js` at https://evoryder8-collab.github.io/APXAppiC/.
- Signed Release build succeeded from the retained `build/APEX-Lane-Task5-integrated` cache with `CURRENT_PROJECT_VERSION=390`. Deep/strict signature verification passed; both phone and Watch bundles report version 390.
- `devicectl` confirmed installation and launch on physical iPhone `A1A6A3B7-CB35-5FE0-ADA7-4924BCB196D6` and physical Apple Watch `F6BE2986-A704-5C82-BC2B-6D02E09CBD04`. No HealthKit test workouts or hydration samples were created. Hardware installation/launch is not a claim of physical watch-face complication verification.

## 2026-09-08 — Onboarding Continue dead-end and keyboard navigation

- Reproduced the owner's Normal week blocker in the actual Individual preview: selecting daily activity left a second mandatory training-recency question below the fold, while Continue was disabled without explanation and still looked actionable. Continue now reveals and names the first missing answer rather than silently stopping; the validation itself is unchanged. The same guidance covers movement tabs, setup frequency/time and explicit safety acknowledgment. New stages reset their scroll position, not their answers.
- The driven flow also caught missing Previous/Next/Done keyboard toolbar controls in the standalone preview presentation. Moved these existing localized controls into the onboarding footer while a body field is focused, making numeric entry independent of a navigation toolbar host.
- One retained UI journey, not a broad regression suite: initial keyboard failure reproduced; after repair the same run reproduced Normal week; final repaired journey passed `1/1` in 35.385 seconds (`Test-APEX-2026.09.08_04-19-43-+0200.xcresult`, retained APEX lane build cache). It completes blank-answer onboarding, all required questions, starting-map review, local plan creation, optional-permissions continuation, portal entry and return to My account. Root inspected recency-guidance and populated-home screenshots. Existing validation/math/account data were not changed; no real Health samples, accounts or subscription charges were created.
- Safety guidance is authored in every offered language; other prompts reuse existing translations. Scope and honest limits are in `docs/superpowers/plans/2026-09-08-onboarding-walkthrough.md`. This is not an exhaustive app audit, hardware-sensor check or proof of real subscription purchase/backend onboarding.
- Signed Release 391 build succeeded; both phone and Watch bundles report 391, and strict deep signature verification passed. Both devices confirmed successful installation. iPhone launch succeeded; the subsequent Watch launch command timed out establishing its device tunnel, so automatic Watch launch is not confirmed (installation is confirmed).
- Implementation commit `b27394a9e81c3cbdb3549bbfcd9cf51b07303bf2` was pushed to both required refs. GitHub Pages run `34179909666` succeeded; the live site returned HTTP 200. The wider audit backlog remains open.

## 2026-09-09 — Generated sessions respect available time

- Continued the documented open session-time finding. Both native and web recorded available minutes but generated the same workload regardless. Targeted tests reproduced the defect before production edits.
- Newly generated sessions now fit the selected time using existing runnable-duration estimates: remove optional whole groups first, reduce sets without shortening recovery or tempo, then omit trailing whole groups if necessary. Minimum-effective rows retain only surviving movements and never exceed their full-session set counts. Absent/invalid limits and generous budgets preserve original prescriptions. Existing stored plans, bespoke programmes, receipts and owner IDs are not migrated or rewritten.
- Focused verification: native `TrainingInductionTests.testAvailableTimeConstrainsGeneratedSessionsWithoutShorteningRest` passed 1/1 after red proof on both home and gym; all 28 tests in the affected web induction module passed; production web build and diff checks passed. Home/gym generation probes for 15/30/45-minute inputs stayed within their displayed budgets. One independent scoped review found no actionable defects. No broad regression suite or production Health data mutation.
- Duration remains an estimate, not a stopwatch guarantee. Native retains its existing rounded-minute estimate; web uses its seconds estimate. Longer budgets do not invent extra volume. Broader goal-specific prescription personalization and calorie-calibration cadence remain separate open findings.
- Published implementation `0ea3418ba475bce94a68e321e6b104e578c1d2d0` to both refs; Pages run `34350016299` succeeded and live entry `assets/index-C0u4TC1O.js` was confirmed. Signed Release 392 built and passed strict deep signature verification, then installed successfully on the iPhone. Watch installation was blocked by device lock (CoreDevice 10003 / RemotePairing 1016); no Watch update is claimed. Unlocking the Watch is required to finish that installation. The retained build cache was reused.

## 2026-09-09 — Live coach invitation crypto hotfix

- Owner reported `function gen_random_bytes(integer) does not exist`. Authenticated production SQL inspection confirmed pgcrypto in `extensions`, while all three invitation RPCs restrict search_path to `public, pg_temp`. A rollback-only real create/preview/accept test reproduced the exact failure at token generation.
- Applied migration 051 directly through the authenticated APEX Supabase SQL editor (no blind db push or migration-history rewrite). It explicitly qualifies random-byte and SHA-256 helpers in creation, preview and acceptance, preserving function bodies, ownership, grants, SECURITY DEFINER and restricted search paths. One independent scoped review found no migration blockers.
- The real production create → preview → accept → reject reused token regression then succeeded. Token shape, consented workout-only scope, active relationship and absent visual consent were asserted. All test mutations rolled back; readback found zero regression users and invitations. No invitation email was sent, no client account/seat remains. This verifies the invitation lifecycle, not every coach feature.
- After the owner unlocked the Watch, signed build 392 installed successfully. The invitation repair is server-side and immediately available to the installed phone app. The separate tested calibration guard change remains staged for its own commit/release.

## 2026-09-09 — Weekly calorie-calibration cadence parity
...
## 2026-09-09 — Invitation success feedback and honest delivery handoff

- After the server fix, invitation creation returned a token but the native sheet only appended a ShareLink under the long form. There was no confirmation/navigation and no automatic email implementation. Reproduced the missing confirmation with the actual Developer Mode coach UI journey.
- Success now replaces the form with a visible Invitation ready screen, recipient and prominent private-link sharing action; it explicitly states no email was automatically sent. Resetting the scroll position makes the next step visible. The create action disappears to avoid accidentally revoking the just-created link. Keyboard is dismissed on submission and fields/scopes are disabled while pending so the displayed recipient cannot diverge from the request. Owner-generation guards remain intact.
- Authored both new strings in all nine native languages. The focused UI journey failed before the fix and passed afterward (1/1); its captured confirmation screen was inspected. One scoped review identified the editable-recipient race, closed with pending-state disabling; Release compilation covers that final guard. No full regression suite or real invitation/email was generated by this UI test.
- Automatic transactional email delivery is still not implemented. The supported next step is Share private invite via the system share sheet (Mail/Messages/copy as available). This UI fix does not claim a mail-sending backend exists. Release 394 delivery follows.

### Release 394 delivery
...
## 2026-09-19 — Explicit developer UI modes and sponsored-client preview

- Extended the existing authorized-owner-only Developer Mode as UI Mode: Individual subscribed user, Coach/PT Interface, Coach's client, Trial (active), Non-subscribed (no access), plus My account. Two separate trial/access states avoid conflating an active trial with denied access. Trial is a disposable presentation simulation, not a new commercial trial offer or payment implementation.
- Added sample-local access presentation. Membership previously read the real founding owner's entitlement, and the preview hid its membership card altogether; both are corrected. Sponsored-only sample clients keep nutrition/Avatar/coach-plan access and cannot access personal builders or coach administration. No-access previews render the existing access-recovery screen without the private portal underneath; retry explains that no server request was made. Sample invitation acceptance can grant sample sponsorship without individual subscription access.
- The real authenticated session, server entitlement store, photos, Health/device connections and billing remain isolated. Preview alerts are now presented; permanent My account return remains above sheets. All new copy is authored in nine native languages, including five compact mode labels. No web preview implementation is claimed by this native slice.
- The new trial/access test failed first with the absent role. After implementation, all 8 sandbox behavioral tests plus the 9-language compact-table test passed (9/9). One scoped read-only review caught the hidden membership card, corrected before UI walkthroughs. The review found no remaining significant issue; it is not an exhaustive coach-platform audit. UI journey/release evidence follows.
- The client walkthrough exposed preview chrome covering native Back navigation; screenshot review found the same problem with the coach invitation sheet's Close control. Both containers now reserve the strip's space outside their navigation content. Preview membership sits immediately beneath identity, and sample identities no longer display a bespoke persona badge. Simple and Advanced use the same coach-plan accessibility identifier.
- The first complete client journey passed sponsorship display, acknowledgement, activation, published programme visibility, trial setup, locked-access feedback and return to the unchanged bespoke identity. Final focused journeys also check the invitation Close control's clearance and open the published workout's session controls. All 17 changed string tables passed `plutil -lint`; no broad unrelated regression run was added.
- Final UI proof: both targeted journeys passed (2/2, 106.3 s) on `APEX Lane · iPhone 17 Pro`, with coach invitation Close clear of the preview strip and the client reaching workout session controls. Result: `test_sim_2026-09-19T12-23-30-664Z_pid76753_7417413d.xcresult` in the retained XcodeBuildMCP result-bundles directory; screenshots exported to ignored `build/ui-mode-396-captures`. These are isolated sample-flow checks, not proof of commercial billing, email delivery or production coach/client synchronization.
- Release evidence: source `12fb4d0e0a2b4d5776d6b7c902024b04bcc34bc2` built signed Release **396** using the retained integrated DerivedData. iPhone and Watch bundles report 396 and both pass deep/strict signature verification with system trust-store access. Build completed in 167.7 s; pre-existing concurrency warnings remain outside this focused slice. iPhone installation succeeded, independent CoreDevice inventory confirms 396, and APEX launched as PID 2002.
- Watch delivery is **not confirmed**: direct installation timed out (`Network.NWError 60`); one independent inventory check failed mounting/reading the Watch developer disk-image metadata (`CoreDevice 12040/12044/12018`). The signed 396 companion is embedded in the installed phone build. No Watch launch was requested. Reconnect/unlock the Watch and restore its developer connection before retrying direct delivery; do not mistake discovery as proof of installation.
- Both required refs received the implementation SHA. GitHub Pages run `35442958720` succeeded; `https://evoryder8-collab.github.io/APXAppiC/` served the APEX page and its referenced `index-nw-DsKmY.js` returned HTTP 200. This follow-up entry records delivery evidence; no implementation changed after the tested build.
- Recorded the user's coach/sponsored-client polish requirements as an explicit Phase 6 readiness gate. Production transactional email, purchases, backend permissions and cross-device coach delivery require separate real integration verification; disposable previews cannot certify them.

## 2026-09-19 — Deliver Constantine's missing V8.5 morning circles natively

- Confirmed the actual iPhone's account cache still held seed version 7, protocol version 83 and the V8.3 main programme, despite the authored V8.5 seed already existing on web. The upgrade was only reached through the web store; installing a native binary did not deliver it. The read-only account cache copy remains private and outside the repository.
- Added a narrowly scoped native 7-to-8 delivery for the authorized Constantine bespoke account. A checked, generated bundle shares the web V8.5 prescription (18 main-phase sessions, 97 Full/Light exercise definitions). It creates stable owner-scoped replacement cards, preserves the original protocol start date, and retires old authored cards without deleting their exercises, workout receipts or logs. Other owners, personal/questionnaire plans, pending edits, custom/recovery/coach content and unrelated programme data are excluded.
- Delivery stages only absent inactive cards, writes exercises before activation, and advances the seed marker last. Every network phase verifies the authenticated owner. Duplicate staging is ignored server-side, so retries or concurrent stale preparations cannot hide an already installed card. Protected original IDs follow the replacement; legacy archive membership cannot leave a protected old card duplicated alongside it.
- Rechecked the supplied `Constantin_Training_V8.5_30Aug2026.pdf`, especially its morning-circle addendum. Seven morning cards rotate the eligible targets and precede the day's separate official/T25 sessions. Full mode has five sets per eligible target; Light and scheduled deloads retain two. The player now performs one set of each movement per round, preserving its authored rest, rather than five consecutive sets of one movement. Both web and native timelines use the same behavior. This implements the owner's authored programme, not a claim that its experimental high volume is optimal for everyone.
- Replaced the obsolete main-phase description with accurate morning/strength/T25 copy in all offered languages. No user data, programme start date, nutrition targets or account identity is reset.
- Focused red proofs covered the missing native bundle/version, wrong circuit sequence/rest, retry restaging and protected/archive duplication. Final native unit checks passed 12/12; affected web programme/seed/player/history checks passed 25/25. Manifest drift check, eight translation table validations, production web build and whitespace checks passed. A bounded independent review cleared its three findings after fixes. The UI walkthrough and signed iPhone delivery evidence follow; no broad unrelated regression suite or Health samples were generated.
- UI verification passed 1/1 (46.8 s) on the dedicated APEX simulator after correcting the test's calendar-sheet action selector. The journey opens today's AM card, shows the three Saturday exercises at five sets each, and starts its guided player at round 1 of 5. Captures were visually inspected in ignored `build/v85-morning-397-captures`; result bundle `test_sim_2026-09-19T21-41-16-525Z_pid23327_baa6a2f7.xcresult`. Saturday's separate Push B session remained visible; no simulated workout was saved to a real account.
- Signed Release **397** built successfully (343.7 s), passed strict/deep signature verification, and contains the exact checked programme bundle. Installation over the existing physical iPhone app succeeded; independent CoreDevice inventory reports 397, and launch succeeded (PID 5035). No Watch-specific implementation or direct Watch installation was needed for this phone programme delivery.
- Post-launch readback of the actual account confirms seed **8**, protocol **85**, all **18 visible main-phase sessions**, **7 morning circles**, and all seven old authored main cards inactive. Every morning Full exercise has five sets, every Light exercise two, and every morning exercise has its circuit group. All **572** pre-existing exercise records and both workout-history collections are unchanged; original start date **2026-07-25** is preserved. This closes the real delivery failure, not merely the simulator fixture. GitHub/Pages publication verification follows.
