...
export function RoutePlanner() {
  ...
  return (
    <OrbitFrame title="Route planner" subtitle="Build a route around the mission, not only the distance." backTo="/orbit">
      <div className="space-y-4">
        <GlassCard accent={ACCENTS.ice} className="p-5 sm:p-6">
          <p className="mb-2 text-[11px] font-bold tracking-wide text-ink-faint uppercase">{t('Choose mission')}</p>
          <MissionPicker value={mission} onChange={setMission} />
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            <div className="sm:col-span-2">
              <div className="flex gap-2" role="group" aria-label={t('Plan by distance or duration')}>
                {(['distance', 'duration'] as PlanningMeasure[]).map((item) => <button key={item} type="button" onClick={() => setMeasure(item)} className={`rounded-full px-3 py-1.5 text-[11px] font-bold ${measure === item ? 'bg-sky-950 text-sky-100' : 'bg-white/65 text-ink-soft'}`}>{item === 'distance' ? t('Distance') : t('Duration')}</button>)}
              </div>
              <div className="mt-1.5 flex min-h-12 items-center rounded-2xl border border-white/80 bg-white/70 px-3">
                {measure === 'distance' ? <>
                  <input aria-label={t('Desired distance in kilometres')} type="range" min="1" max="42.2" step="0.5" value={distance} onChange={(event) => setDistance(Number(event.target.value))} className="w-full accent-sky-500" />
                  <span className="ml-3 min-w-16 text-right font-mono text-sm font-bold text-sky-700">{distance.toFixed(1)} km</span>
                </> : <>
                  <input aria-label={t('Desired duration in minutes')} type="range" min="15" max="240" step="5" value={duration} onChange={(event) => setDuration(Number(event.target.value))} className="w-full accent-sky-500" />
                  <span className="ml-3 min-w-24 text-right font-mono text-sm font-bold text-sky-700">{duration} min</span>
                </>}
              </div>
              {measure === 'duration' && <p className="mt-1 text-[10px] text-ink-faint">{t('Orbit will seek roughly')} {requestedDistance.toFixed(1)} km {t('for this mission, then show each option’s estimate.')}</p>}
            </div>
            ...
            <label className="flex min-h-12 items-center justify-between rounded-2xl border border-white/80 bg-white/70 px-3 text-sm font-bold text-ink">
              {t('Simple navigation')}
              <input type="checkbox" checked={simple} onChange={(event) => setSimple(event.target.checked)} className="h-5 w-5 accent-sky-500" />
            </label>
          </div>
        </GlassCard>
        ...
        <GlassCard className="p-5">
          <div className="flex flex-wrap gap-2">
            <GhostButton onClick={() => void useCurrent()}>{t('Use current location')}</GhostButton>
            <GhostButton onClick={() => { setManual(true); setManualPoints(start ? [start] : []); setSelected(null) }}>{t('Draw manually')}</GhostButton>
            <GhostButton onClick={() => fileRef.current?.click()}>{t('Import GPX')}</GhostButton>
            <input ref={fileRef} type="file" accept=".gpx,application/gpx+xml,text/xml" className="hidden" onChange={(event) => void onFile(event.target.files?.[0])} />
          </div>
          ...
          <div className="mt-2 flex gap-2 overflow-x-auto pb-1" aria-label={t('Search result purpose')}>
            {(['start', 'destination', 'waypoint'] as SearchRole[]).map((role) => <button key={role} type="button" onClick={() => setSearchRole(role)} className={`shrink-0 rounded-full px-3 py-1.5 text-[10px] font-bold uppercase ${searchRole === role ? 'bg-sky-950 text-sky-100' : 'bg-white/65 text-ink-soft'}`}>{t(role === 'start' ? 'Start' : role === 'destination' ? 'Destination' : 'Waypoint')}</button>)}
          </div>
          ...
          }} className="w-full rounded-xl bg-white/60 px-3 py-2 text-left text-xs font-semibold text-ink-soft">{result.label}</button>)}</div>}
          {(start || destination || waypoints.length > 0) && <div className="mt-3 flex flex-wrap gap-2 font-mono text-[10px] text-sky-700">
            {start && <span>{t('Start').toUpperCase()} {start.lat.toFixed(4)}, {start.lng.toFixed(4)}</span>}
            {destination && <button type="button" onClick={() => setDestination(null)}>{t('Destination set').toUpperCase()} ×</button>}
            {waypoints.length > 0 && <button type="button" onClick={() => setWaypoints([])}>{waypoints.length} {t(waypoints.length === 1 ? 'Waypoint' : 'Waypoints').toUpperCase()} ×</button>}
          </div>}
          <label className="mt-3 block text-[10px] font-bold tracking-wide text-ink-faint uppercase">{t('Roads or areas to avoid')}
            <input value={avoidNotes} onChange={(event) => setAvoidNotes(event.target.value)} placeholder={t('Comma-separated notes')} className="mt-1.5 min-h-11 w-full rounded-2xl border border-white/80 bg-white/70 px-3 text-xs font-semibold normal-case tracking-normal text-ink" />
          </label>
          {message && <p role="status" className="mt-3 rounded-2xl bg-sky-50/80 px-3 py-2 text-xs font-semibold leading-relaxed text-sky-900">{t(message)}</p>}
          <GradientButton accent={ACCENTS.ice} onClick={() => void generate()} disabled={busy || !start} className="mt-4 min-h-13 w-full" breathe>{busy ? t('Working…') : t('Generate routes')}</GradientButton>
        </GlassCard>

        <Suspense fallback={<div className="h-72 animate-pulse rounded-[22px] bg-slate-900/80" />}>
          <OrbitMap
        ...
        {candidates.length > 0 && <div className="grid gap-3 sm:grid-cols-3">{candidates.map((candidate, index) => <button type="button" key={candidate.id} onClick={() => setSelected(candidate)} className={`rounded-3xl border p-4 text-left transition ${selected?.id === candidate.id ? 'border-sky-400 bg-sky-950 text-white shadow-xl' : 'glass border-white/80 text-ink'}`}><OrbitPill tone={index === 0 ? 'amber' : 'ice'}>{index === 0 ? t('Best fit').toUpperCase() : `${t('Option').toUpperCase()} ${index + 1}`}</OrbitPill><p className="mt-3 font-display text-lg font-bold">{formatDistance(candidate.distance_m)}</p><p className="mt-1 font-mono text-[10px] opacity-70">{candidate.estimated_duration_min} {t('MIN')} · {candidate.elevation_gain_m == null ? t('Elevation unavailable').toUpperCase() : `${candidate.elevation_gain_m} ${t('m gain').toUpperCase()}`} · {t(candidate.navigation_complexity).toUpperCase()} {t('NAV')}</p><p className="mt-3 text-xs leading-relaxed opacity-80">{t(candidate.explanation)}</p></button>)}</div>}
        ...
        {selected && !manual && <GlassCard accent={ACCENTS.ice} className="p-5">
          ...
          <div className="mt-4 grid grid-cols-2 gap-2 sm:flex">
            <GradientButton accent={ACCENTS.ice} onClick={() => void startRoute()} className="min-h-13 sm:min-w-44">{t('Start this route')}</GradientButton>
