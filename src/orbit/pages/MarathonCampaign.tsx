...
function CancelCampaignCard({ confirming, busy, onBegin, onKeep, onCancel }: {
  confirming: boolean
  busy: boolean
  onBegin: () => void
  onKeep: () => void
  ...
  const t = useOrbitText()
  return <div className="relative overflow-hidden rounded-[28px] border border-rose-200/70 bg-gradient-to-br from-white via-rose-50/80 to-amber-50/60 p-5 shadow-[0_24px_60px_-42px_rgba(190,24,93,.8)]"><div className="pointer-events-none absolute -right-16 -bottom-20 h-44 w-44 rounded-full bg-rose-300/20 blur-3xl" aria-hidden /><div className="relative"><p className="font-mono text-[9px] font-black tracking-[.2em] text-rose-700 uppercase">{t('CAMPAIGN CONTROL')}</p><h3 className="mt-2 font-display text-lg font-bold text-ink">{t('Cancel marathon campaign')}</h3><p className="mt-1 max-w-2xl text-xs leading-relaxed text-ink-soft">{t('All planned campaign runs will disappear from Orbit and the workout calendar. Completed run history stays private and intact.')}</p>{confirming ? <div className="mt-4 rounded-2xl border border-rose-200 bg-white/75 p-3"><p className="text-sm font-bold text-rose-950">{t('Remove this campaign and every planned run?')}</p><div className="mt-3 grid grid-cols-2 gap-2"><GhostButton onClick={onKeep} disabled={busy}>{t('Keep campaign')}</GhostButton><button type="button" onClick={onCancel} disabled={busy} className="min-h-11 rounded-2xl bg-gradient-to-r from-rose-700 to-red-600 px-3 text-xs font-black text-white shadow-[0_14px_30px_-18px_rgba(190,24,93,.9)] disabled:opacity-50">{t(busy ? 'Removing…' : 'Cancel campaign and remove planned runs')}</button></div></div> : <button type="button" onClick={onBegin} className="mt-4 min-h-11 rounded-2xl border border-rose-300 bg-white/75 px-4 text-xs font-black text-rose-800 transition active:scale-[.98]">{t('Cancel marathon campaign')}</button>}</div></div>
}
...
export function MarathonCampaignPage() {
  ...
  const cancelCurrentCampaign = async (): Promise<void> => {
    if (!campaign || cancelBusy) return
    setCancelBusy(true)
    try {
      await orbit.cancelCampaign(campaign.id)
      app.toast(t('Campaign cancelled. Planned runs were removed.'), 'ok')
      navigate('/orbit', { replace: true })
    } catch {
      app.toast(t('The campaign could not be removed. Try again.'))
      setCancelBusy(false)
    }
  }

  const cancellationCard = <CancelCampaignCard confirming={cancelConfirm} busy={cancelBusy} onBegin={() => setCancelConfirm(true)} onKeep={() => setCancelConfirm(false)} onCancel={() => void cancelCurrentCampaign()} />
  ...
  if (campaign.status === 'review_required') return <OrbitFrame title="Marathon Campaign" subtitle="Professional review recommended before strenuous marathon preparation." backTo="/orbit"><div className="space-y-4"><GlassCard accent={ACCENTS.amber} className="p-6"><OrbitPill contrast tone="amber">{t('FITNESS-READINESS OUTCOME')}</OrbitPill><h2 className="mt-4 font-display text-2xl font-bold text-ink">{t('Professional review recommended before strenuous marathon preparation')}</h2><p className="mt-3 text-sm leading-relaxed text-ink-soft">{t(campaign.assignment_reason)}</p><p className="mt-3 text-xs leading-relaxed text-ink-faint">{t('This is not a diagnosis and APEX has not medically cleared or rejected you. General Orbit route planning and easy free running remain available, subject to any existing professional restriction.')}</p><div className="mt-5 flex gap-2"><GradientButton accent={ACCENTS.ice} onClick={() => navigate('/orbit')}>{t('Use general Orbit')}</GradientButton><GhostButton onClick={() => navigate('/orbit/induction')}>{t('Review answers')}</GhostButton></div></GlassCard>{cancellationCard}</div></OrbitFrame>

  if (campaign.status === 'paused') return <OrbitFrame title="Marathon Campaign" subtitle="More information needed" backTo="/orbit"><div className="space-y-4"><GlassCard accent={ACCENTS.amber} className="p-6"><OrbitPill contrast tone="amber">{t('TIMELINE CHECK')}</OrbitPill><h2 className="mt-4 font-display text-2xl font-bold text-ink">{t('More information needed')}</h2><p className="mt-3 text-sm leading-relaxed text-ink-soft">{t(campaign.assignment_reason)}</p>{campaign.timeline_warning && <p className="mt-3 rounded-2xl bg-amber-50 p-3 text-sm font-semibold text-amber-900">{t(campaign.timeline_warning)}</p>}<GhostButton onClick={() => navigate('/orbit/induction')} className="mt-5">{t('Review induction')}</GhostButton></GlassCard>{cancellationCard}</div></OrbitFrame>
