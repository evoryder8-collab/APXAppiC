...
export function seedSupplements(userId: string): Supplement[] {
  ...
  const rows: Row[] = [
    ...
    { name: 'Casein', dose: 'Normal evening: close the remaining protein gap. Heavy resistance evening: enough powder to provide 40–45 g actual protein.', group: 'As needed', clock: '21:30' },
    { name: 'Citrulline malate', dose: '6–8 g', group: 'Optional training support', offset: -45, trainingOnly: true },
    { name: 'Cluster Dextrin', dose: '25–40 g in 5 g increments when workload or the remaining carbohydrate gap requires it', group: 'Optional training support', offset: -15, trainingOnly: true },
    { name: 'Collagen + Vitamin C', dose: '10–15 g collagen plus a small vitamin C source before tendon-focused work', group: 'Optional training support', offset: -60, trainingOnly: true },
  ]
  return rows.map((r, i) => ({
    id: sid(),
    user_id: userId,
