...
function photoOutbox(userId: string, operation: 'upload_photo' | 'delete_photo' | 'update_photo', photoId: string): PrivateOutboxOp {
  return {
    id: crypto.randomUUID(), user_id: userId, domain: 'photo', operation,
...
export function ProgressPhotoStoreProvider({ children }: { children: ReactNode }) {
  ...
  const hydrate = useCallback(async () => {
    ...
    setReady(false)
    try {
      const local = await privateGetAllForUser<ProgressPhoto>('progress_photos', userId)
  ...
  const sendOperation = useCallback(async (operation: PrivateOutboxOp): Promise<void> => {
    ...
    if (operation.operation === 'upload_photo') {
      ...
      const synced: ProgressPhoto = { ...photo, sync_status: 'synced', updated_at: new Date().toISOString() }
      const { error: metadataError } = await supabase.from('progress_photos').upsert(remotePhoto(synced), { onConflict: 'user_id,client_idempotency_key' })
    ...
    try {
      ...
      for (const operation of operations) {
        try {
          ...
          setPhotos((current) => current.map((photo) => photo.id === operation.entity_id ? { ...photo, sync_status: 'failed' } : photo))
          console.warn('Private photo sync remains queued', error)
          break
        }
      }
    } finally {
  ...
  const savePhoto = useCallback(async (input: SavePhotoInput): Promise<ProgressPhoto> => {
    ...
    const processed = await processProgressPhoto(input.raw)
    ...
    await installLocalUrl(id, false)
    await installLocalUrl(id, true)
  ...
  const value = useMemo<PhotoStoreValue>(() => ({
    ready, syncing, photos, fullUrls, thumbnailUrls, savePhoto, deletePhoto, updatePhoto, setCrop, ensurePhotoUrl,
  }), [deletePhoto, ensurePhotoUrl, fullUrls, photos, ready, savePhoto, setCrop, syncing, thumbnailUrls, updatePhoto])
